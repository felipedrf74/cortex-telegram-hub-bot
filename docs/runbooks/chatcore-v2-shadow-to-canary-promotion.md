...
2. **Kill-switch wins — but is NOT yet a single chokepoint (see GATE-P1).** Today, `off` wins on the live path only because **two** parsers each independently short-circuit on the literal `off`: `resolveChatCoreV2LocalChatLlmMode` (`local-chat-orchestrator.ts:119`) and `resolveChatCoreV2ActionGatewayMode` (`action-gateway.ts:105`) both run `String(env.CHAT_CORE_V2_ORCHESTRATOR_MODE ?? '').trim().toLowerCase() === 'off'`. The activation-config collapse (`activation-flags.ts:63-77`) is a **separate** code path that the live chat mode decision does not consume for its shadow/canary/on choice. WP-00.5 must consolidate these into one `resolveChatCoreV2ActivationConfig(env).mode` chokepoint before this runbook can proceed (precondition GATE-P1, Section 2).
...
|---|---|---|---|
| **GATE-P1** | Kill-switch consolidated into ONE chokepoint; both live parsers delegate to `resolveChatCoreV2ActivationConfig(env).mode`; no inline mode compare against any env var remains in `local-chat-orchestrator.ts`/`action-gateway.ts`. | WP-00.5; peer review item 10 | **Gate = the integration test, not the grep.** `__tests__/api/chat-core-v2-live-path-killswitch.integration.test.ts` green: with mode `off`, both `runChatCoreV2ActionGateway` and `runChatCoreV2LocalChatTurn` are no-ops on the live `chat-message-routes` path. **Smell-check only (non-gating):** `grep -rnE "(process\.env\|env)\.CHAT_CORE_V2_ORCHESTRATOR_MODE" src/services/chat-core-v2/local-chat-orchestrator.ts src/services/chat-core-v2/action-gateway.ts` returns **zero** AFTER WP-00.5 reroutes both. (NB: today this grep returns **two** hits — `:119` and `:105` — both using the `env` parameter, not `process.env`; a `process.env`-only grep is a false-negative trap and must never be the gate.) |
| **GATE-P2** | Layer-1 prepass + `ChatTurnPlanMicro` schema/repair wired into the live `route-decision.ts` path (not the barrel re-export). | WP-01; peer review item 1 | `__tests__/services/chat-core-v2-prepass-route-decision-wiring.test.ts` green; a real `planChatCoreV2ShadowTurn` persists a `kind='custom'`, `name='prepass_candidate_selection'` span via the real `recordChatV2TraceSpan`. |
...
> **Env model (verified, BLOCKING correction):** canary is **NOT** turned on by `CHAT_CORE_V2_ORCHESTRATOR_MODE=canary` alone. The live chat path resolves through `resolveChatCoreV2LocalChatLlmMode`, which reads the **separate** `CHAT_CORE_V2_LOCAL_CHAT_LLM_MODE` var (`local-chat-orchestrator.ts:120`), and in production hard-requires `CHAT_CORE_V2_LOCAL_CHAT_ALLOW_PROD === '1'` (`:137`). The action gateway reads `CHAT_CORE_V2_ACTION_GATEWAY_MODE` (`action-gateway.ts:106`). Setting only `ORCHESTRATOR_MODE=canary` resolves the local-chat LLM mode to `off` (`:122` default), so canary would silently never serve and Step 6's success criterion would fail unexplained. Until WP-00.5 collapses these into `ORCHESTRATOR_MODE`, the **full tuple below is the only correct sequence**.
...
CHAT_CORE_V2_LOCAL_CHAT_LLM_MODE=canary                      # REQUIRED — the actual local-chat mode source (local-chat-orchestrator.ts:120)
CHAT_CORE_V2_ACTION_GATEWAY_MODE=shadow                      # read-only canary keeps the gateway in shadow (NOT enforce) (action-gateway.ts:106)
CHAT_CORE_V2_LOCAL_CHAT_ALLOW_PROD=1                         # production-only hard gate (local-chat-orchestrator.ts:137); harmless on staging
CHAT_CORE_V2_LOCAL_CHAT_CANARY_USERS=<tenantId:userId,...>   # explicit allowlist (parseCanaryList, local-chat-orchestrator.ts:138)
...
> NOTE: if/when WP-00.5 collapses the multi-var model into a single `ORCHESTRATOR_MODE`, this tuple reduces — but the single-flag model is **BLOCKED ON GATE-P1**. Use the tuple until then.
...
```
**Resolution path as it actually exists pre-WP-00.5 (honest):** the live mode for the chat path is resolved by `resolveChatCoreV2LocalChatLlmMode` (`local-chat-orchestrator.ts:119`) and `resolveChatCoreV2ActionGatewayMode` (`action-gateway.ts:105`), each reading **its own** env var, and each short-circuits to `off` only on the literal `CHAT_CORE_V2_ORCHESTRATOR_MODE=off`. The activation-config collapse (`activation-flags.ts:63-77`) is a **separate** code path. So "`off` wins" is true today because of the **two duplicated inline `off`-checks**, not because of a single chokepoint — that consolidation is GATE-P1/WP-00.5. The in-process resolver re-reads env only on **process restart**; there is no `.env` hot-reload today, so the restart is mandatory. This matches the Phase-2 WO rollback contract (`WO-chatcore-v2-production-activation.md:399` — "set `CHAT_CORE_V2_ORCHESTRATOR_MODE=off`").
...
6. **The kill-switch is not yet a single chokepoint (GATE-P1 open).** `local-chat-orchestrator.ts:119` and `action-gateway.ts:105` each still do an inline `String(env.CHAT_CORE_V2_ORCHESTRATOR_MODE ?? '').trim().toLowerCase() === 'off'` compare — the two parsers WP-00.5 must reroute. A `process.env`-only grep returns zero (false-negative); the real gate is the integration test.
