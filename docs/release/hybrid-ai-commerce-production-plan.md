...
- The surviving acceptance-v2 inventory may cross that boundary only through the reviewed acceptance-v3 tool's one-way pre-smoke migration. It validates the exact ordered rows under the v3 schema, refuses any legacy bare source assertion or existing smoke identity, and atomically persists the receipt-backed binding before its first API request. Evidence generation never upgrades or repairs state after smoke submission. An exhausted seven-pass/two-failure predecessor remains immutable historical evidence. Its separately reviewed v4 successor must digest-bind the exact predecessor bytes, carry only the seven contract-valid outputs, create only the two failed scenarios plus the pristine smoke under the successor request revision, and revalidate the exact predecessor file during successor execution, smoke launch, and evidence-v6 production; it cannot mutate or present the predecessor as complete.
