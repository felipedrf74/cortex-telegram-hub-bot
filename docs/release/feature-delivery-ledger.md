...
|---|---|---|---|---|---|---|---|---|---|
| `beta_registry_v1` | Public beta registry double opt-in, MX/disposable validation, 30-day invite email approval, DB invite redemption, and static reviewer-code expiry policy | in_worktree | codex/beta-registry-stripe-billing | pending | pending commit | `waitlist-routes.test.ts`, `portal-waitlist-routes.test.ts`, `email-sender.test.ts`, `user-service.test.ts`, auth/entitlement coverage | pending worktree evidence | 2026-05-18 | Production launch waits for Resend/domain configuration and staging smoke. |
...
| `stripe_web_checkout_v1` | Public website Stripe Checkout for Pro/Max monthly USD/BRL plus webhook idempotency and verified-user checkout claim | in_worktree | codex/beta-registry-stripe-billing | pending | pending commit | `billing-routes.test.ts`, public checkout tests, `stripe-service.test.ts`, Stripe webhook idempotency/claim tests | pending worktree evidence | 2026-05-18 | Sandbox Prices created: Pro USD `price_1TYUtmEnGIEp1Q5vqsfLN9Ml`, Pro BRL `price_1TYUtnEnGIEp1Q5vMfu5XXt1`, Max USD `price_1TYUtoEnGIEp1Q5vievUfmeu`, Max BRL `price_1TYUtpEnGIEp1Q5vtuAejLdn`. |
