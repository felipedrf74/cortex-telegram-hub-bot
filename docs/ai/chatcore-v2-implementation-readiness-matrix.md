...
| Layer 1 assembly map | complete for docs plus pure selector helper | `chatcore-v2-layer1-assembly-map.md`, `src/services/chat-core-v2/prepass-candidate-selection.ts` | Runtime integration blocked by D1-D16 peer review and Phase 2 shadow prerequisites |
...
| iOS turn-state contract | first helper implemented; sandbox UX bridge partial | Phase 1 contracts, `src/services/chat-core-v2/turn-state-events.ts`; iOS slow-answer progress copy updated in `ChatViewModel.swift` | Full reconnect/background event implementation still pending |
...
|---|---|---|---|
| Phase 2 plan-only shadow | impossible now | Serialized D3 latency passes the operator-revised gate, but burst/concurrent/sustained p95 is still ~10-15s on CPU-only Ollama; D1-D16 are not peer-reviewed; shadow table, repair loop, privacy storage, and rollout tests are not implemented | Wire the new queue/concurrency policy into shadow runtime, implement shadow storage/repair/privacy tests, peer-review D1-D16 |
...
| WebSocket reconnect sequenceNumber resume | sequence helper test complete; server/iOS runtime blocked | `src/services/chat-core-v2/turn-state-events.ts`; requires full iOS/server progress event implementation |
...
| HMAC-only shadow/cloud identifiers | cloud/prepass HMAC helpers complete; action-gateway telemetry avoids hardcoded fallback secrets; shadow-route hook now HMAC-only and skips recording without a secret; shadow replay/trace persistence implemented + redacted + HMAC + retention, gated default-off | `src/services/chat-core-v2/cloud-allowlist-packet.ts`, `src/services/chat-core-v2/prepass-miss-log.ts`, `src/services/chat-core-v2/action-gateway.ts`, `src/services/chat-core-v2/shadow-route-hook.ts`, `src/services/chat-core-v2/shadow-gate-readiness.ts`; `chat_v2_replay_bundles`/`chat_v2_trace_spans` self-create and persist redacted HMAC-only rows (no-raw-storage tests), wired + gated default-off so no rows accumulate by default |
| No raw private strings in cloud packets | allowlist packet helper and safe generic-answer packet producer tests complete; runtime egress remains flag-blocked by default | `src/services/chat-core-v2/cloud-allowlist-packet.ts`, `src/services/chat-core-v2/cloud-allowlist-answer-packet.ts`; produced packets contain HMAC-scoped IDs/fingerprints and coarse capability IDs only |
| Queue fallback under local LLM saturation | packet-only dispatcher and safe generic-answer packet producer complete for Phase 2 slice; broader domain answer packets still blocked | `src/services/chat-core-v2/queue-fallback-policy.ts`, `src/services/chat-core-v2/cloud-allowlist-answer.ts`, `src/services/chat-core-v2/cloud-allowlist-answer-packet.ts`, `src/services/chat-core-v2/local-chat-orchestrator.ts`; cloud only when activation allows fallback, queue pressure crosses threshold, cloud mode is enabled, and a positive allowlist packet is safe. The producer is opt-in, budget/HMAC-gated, denies private/app-state requests, denies sensitive personal/medical/legal-distress topics (deny-when-uncertain), and does not send raw message text. The queue-fallback observability event carries only safe enums (kind, reasonCode, cloudDenialReason, routeMethod, surface, locale) through the failure-observability sanitizer |
| Phase 2 shadow gate readiness | shadow persistence + redacted storage + table-level no-raw tests complete; read-only gate-readiness report measures the thresholds; Phase 2 gate UNMET | `src/services/chat-core-v2/shadow-gate-readiness.ts`, `src/services/chat-core-v2/shadow-route-hook.ts`, `src/services/chat-core-v2/trace-recorder.ts`, `src/services/chat-core-v2/model-run-audit.ts`; `evaluateChatCoreV2ShadowGateReadiness` reports rowCount / schemaValidPct / safeShapeViolations against >=50 rows / >=99% schema / 0 raw-shape. `gateMet` is hard-coded false until recall@8 on a peer-reviewed labeled corpus AND >=50 real shadow rows accumulate (shadow runtime is default-off, so 0 rows by default) |
| Phase 2 recall@8 measurement | read-only Layer-1 prepass recall@8 harness complete; synthetic baseline measured; gate UNMET | `src/services/chat-core-v2/prepass-recall-eval.ts`; `evaluateGoldenCorpusPrepassRecallAtK` over the seed corpus = recall@8 **0.563** (148/263 scored). `selectPrepassCandidateCapabilities` is a contract helper (not yet live-wired) and under-covers cooking/content/write capabilities (top misses: `cooking.recipe_answer`, `content.draft_assist`). SYNTHETIC baseline only — the gate needs a peer-reviewed labeled corpus, and recall must NOT be tuned to the synthetic corpus (overfitting) |
...
| Provider-aware task deterministic read | complete for sandbox scope | `listTasksForUser` merges `native_tasks` and local/unified Nexus rows for native users; task summary deterministic route uses it; pinned by task-service, deterministic-read, and chat-route tests. |
| iOS chat task-create server routing | complete for sandbox scope | `ChatViewModel` routes natural-language task creation through `/api/v1/chat/message` instead of the local REST fastpath; focused and full iOS unit tests pass. |
| iOS recipe parser/progress fixes | complete for sandbox scope | `MessageBubble` handles labelled title output and skips section-label ingredients; long local-LLM progress copy avoids internal action/idempotency wording. |
...
| Full `npm run verify` for current branch | complete | 741/741 test files and 10,894/10,894 tests passed after the provider-aware task read, iOS sandbox bridges, shadow-route HMAC fix, write-intent HMAC fallback removal, D3 queue-fallback local-chat preflight, packet-only cloud allowlist answer dispatcher, safe generic-answer packet producer wiring, the sensitive-topic deny gate, queue-fallback observability enums, the read-only shadow gate-readiness report, and the Layer-1 prepass recall@8 measurement harness |
