...
| Off-host backup durability | ACCEPTED_RESIDUAL_RISK | Backups are encrypted but remain on the same release-host disk. They protect bad-release, operator-error, and corruption recovery, not total NVMe/server loss. A USB SSD, NAS, or another non-AWS host is deliberately deferred for the current project size; no AWS service is a backup or release dependency. |
