...
`BLOCKED_WITH_EXACT_REASON`. There are no intentionally hidden open tasks in
this local pass.
...
| Backup encryption/off-host copy/restore drill | BLOCKED_WITH_EXACT_REASON | Requires access to production backup storage and an approved restore-drill target. Documented in the runbook only. |
...
| 4. Infrastructure, edge, and data protection | DONE for docs/runbooks and local backup tests where available; Cloudflare/VPS/firewall, backup restore drill, and SQLCipher migration remain BLOCKED_WITH_EXACT_REASON. |
...
| 8. Incident response and privacy operations | DONE for repository artifacts: runbook and breach checklist exist. Quarterly restore/tabletop execution is an operations activity and remains BLOCKED_WITH_EXACT_REASON through the backup/ops rows above. |
...
| Infra/CI secret scan, dependency audit, CodeQL, firewall checklist, backup restore drill | DONE for repository/CI: secret-pattern scan, `npm audit`, `pip-audit`, CodeQL workflow, Scorecard workflow, Dependabot config, and runbook checklist exist. Firewall changes and restore drill are BLOCKED_WITH_EXACT_REASON because they require live ops. |
