...
|---|---|---|---|---|---|
| Gap 1: Live TestFlight smoke | Complete with manual verification required | iOS worktree has smoke matrix, manual checklist, local smoke script, focused smoke tests, 2026-04-25 simulator smoke with Felipe's persisted account, backend production live at `4.14.74`, and staging live at `4.14.73`. | Signed TestFlight/device smoke, APNs proof, fresh auth, true two-account switching, and latest Content/Training TestFlight bugfix proof. | Physical-device/TestFlight checklist. | Backend is ready for the smoke gate; iOS distribution proof remains. |
...
- Second Training TestFlight bugfix pass is deployed in backend `4.14.68`,
  ...
  signed TestFlight/device pass before closing user-facing QA.
- Content script AI delivery fixes remain live in backend `4.14.74` and
...
- Training coach engine hardening is deployed in backend `4.14.74`. It removes
  ...
  `docs/beta/training-coach-engine-hardening-handoff.md`. Staging smoke passed
  17/17, production deploy gate passed 345 files / 5,468 tests, and production
  health checks passed for content engine, status portal, and bot online.
