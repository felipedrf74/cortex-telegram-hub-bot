...
|---|---|---|---|---|---|
| Gap 1: Live TestFlight smoke | Complete with manual verification required | iOS worktree has smoke matrix, manual checklist, local smoke script, focused smoke tests, 2026-04-25 simulator smoke with Felipe's persisted account, and backend production/staging live at `4.14.71`. | Signed TestFlight/device smoke, APNs proof, fresh auth, true two-account switching, and latest Content/Training TestFlight bugfix proof. | Physical-device/TestFlight checklist. | Backend is ready for the smoke gate; iOS distribution proof remains. |
...
- Second Training TestFlight bugfix pass is deployed in backend `4.14.68`,
  ...
  signed TestFlight/device pass before closing user-facing QA.
- Content script AI delivery hotfix is deployed in backend `4.14.71` and
  ...
  `/api/v1/internal/ai-complete` proxy passes `jsonMode` and long content
  ...
  standard cache-v6 request. Production script smoke should return
  `degraded=false` unless a real provider outage is occurring.
