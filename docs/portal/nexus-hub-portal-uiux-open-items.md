...
4. **Cascade expansion — ~~OI-ADM-302b~~ [DONE · 2026-04-24 for scheduler; OI-ADM-302c tracks forced-logout follow-up].**
   ...
   - **Forced iOS logout — tracked as OI-ADM-302c.** Existing JWTs stay valid until natural expiry; tenant-scoped calls still 403 via the guard, so the user sees "Your workspace is suspended" rather than "logged out." A true revoke would need: (a) a `session_revocations` table keyed on user_id + revoked_at_epoch, (b) `authMiddleware` checks `issued_at < revoked_at → 401 SESSION_REVOKED`, and (c) iOS-side handling of SESSION_REVOKED vs TOKEN_EXPIRED. (c) is the real blocker — iOS client rollout needs to coordinate with backend before this lands in prod.
