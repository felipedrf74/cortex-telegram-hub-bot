...
- ~~**OI-DATA-003e** — per-key audit history~~ **DONE 2026-04-24.** Every skill-config field now has a 🕐 button in its label that opens a shared `#configHistoryModal` showing the last 20 saves that touched THAT specific key. Values never leak — only actor email/id + timestamp + which OTHER keys were touched in the same save ("Also touched: default_platform"). Uses SQLite's `json_each(json_extract(details, '$.keysTouched'))` for strict key-membership filtering (not a substring LIKE match, which could false-positive). No new schema — rides on the existing `audit_trail` rows written by `writeWorkspaceAudit` at skill-config PUT time. Service: `listSkillConfigHistoryByKey(tenantId, skillId, key, limit)` in `tenant-skill-config-service.ts`. Route: `GET /workspace/skills/:skillId/config/history?key=&limit=`. UI: boot-time `attachHistoryButtonsToConfigFields()` walks the 5 skills' `*ConfigFieldIds()` maps and injects a 🕐 button into each field's `<label>`, so the 30 fields instrument themselves without hand-editing 30 labels. 13 route pins (auth + validation + newest-first order + VALUES-NEVER-LEAK with a planted-secret-marker assertion + substring-match negative + multi-key save appears for each key + tenant-isolation) + 20 UI pins (modal structure, field-key-map helper, boot injection idempotency, fetch shape, 3 result states, actor fallback, esc() on every user-controlled interpolation, Escape-to-close, CSS anchors). Closes a question open since OI-DATA-003 ("not worth the schema complexity on spec alone" — turns out zero schema complexity is needed).
...
- ~~**OI-NAV-203c** — end-to-end cold-invitee signup~~ [DONE · 2026-04-24 — 4 of 5 subpoints ship; remaining item is just email-provider wiring]
  ...
  **What shipped.** The full browser-only signup flow: cold invitee receives the magic link → clicks it → their account is created + invite accepted + web-session JWT issued → they land in `/console`. No iOS installation required, no password.
  ...
  5. ~~**Email provider wiring**~~ [DONE · 2026-04-24 for Resend]. `MAGIC_LINK_MAILER=resend` now ships as a real backend. Implementation in `src/services/mailer.ts` (+~160 LOC): `sendViaResend(email)` calls `https://api.resend.com/emails` via native `fetch` with Bearer auth, JSON payload (`from`, `to`, `subject`, `html`, `text`, `reply_to`, `tags`). HTML body is a plain single-column template with a primary CTA button; plaintext fallback for spam-score + accessibility. `escapeHtml` on every user-controlled interpolation (URL, intent label, tenant name). Non-2xx responses surface as `MailerError('SEND_FAILED')` with the Resend JSON body attached to `details` so ops can diagnose domain-verification / rate-limit / API-key issues without log scraping. Network errors (ECONNREFUSED, timeouts) surface as the same error code with `{ backend: 'resend' }`. `smtp` + `postmark` backends remain reserved (throw `BACKEND_UNIMPLEMENTED`). Required env: `RESEND_API_KEY`, optional `MAGIC_LINK_FROM` (defaults to `welcome@nexushub.me`) + `MAGIC_LINK_REPLY_TO`. Prerequisite DNS on `nexushub.me`: SPF TXT + 2× DKIM CNAME (Resend dashboard generates the exact records) + optional DMARC. **Operational test scripts:** `scripts/send-test-magic-link-email.ts` (sends a real email via whichever backend is configured; exits with a clear message when API key is missing) and `scripts/render-magic-link-preview.ts` (writes the HTML body to `/tmp/nexus-magic-link-preview.html` for visual preview without any network call). 5 new mailer tests (fetch-mocked) pinning Bearer auth + payload shape + FROM fallback + HTML escape + 4xx error surfacing + network-error surfacing.
...
4. **Cascade expansion — ~~OI-ADM-302b~~ [DONE · 2026-04-24 for scheduler; OI-ADM-302c tracks forced-logout follow-up].**
   - ~~**Scheduler pause/resume.**~~ [DONE] New `listActiveTenantIds()` service helper (`src/services/tenant-service.ts`) is the single tenant-iteration choke point. Scheduler's `getOwnerTenantIds()` now JOINs `users` against `tenants` and filters `tenants.status='active'` — suspended / archived / trial tenants automatically drop out of cron fan-out for the 3 per-tenant jobs (invoice_collection, amazon_collection, uber_collection) that use it. No touching of individual jobs required. User-scoped helpers (`getActiveUserIds` / `getOwnerUserIds`) intentionally left alone — user-level jobs (chat cleanup, DM notifications) operate on user state independently of tenant status.
   - ~~**Forced session revocation — OI-ADM-302c**~~ [DONE · 2026-04-24 backend-side]. Backend side ships now; iOS-side handling of the `SESSION_REVOKED` error code is a separate iOS-repo change (tracked as **iOS-AUTH-401-SR**). Until iOS ships that, clients treat the new code as a generic 401 — functionally equivalent to token-expired — which is a degradation-free rollout (cold backend, warm iOS keeps working; warm backend, cold iOS treats it as re-auth prompt).

     **Migration 082** — `session_revocations` table. Append-only ledger keyed by `user_id`; each row records `revoked_at`, a free-form `reason` ('tenant.suspend' / 'user.ban' / 'password.reset' / 'security.incident'), and an `actor_user_id`. Hot-path query is "latest revoked_at per user" via the `idx_session_revocations_user_latest` index. Append-only means concurrent INSERTs can never clobber each other — the auth-middleware always reads max(id).

     **Service** (`src/services/session-revocation-service.ts`, ~200 LOC):
     - `revokeSessionsForUser(userId, { reason, actorUserId?, details? })` → one ledger row.
     - `revokeSessionsForTenant(tenantId, { ... })` → cascades to every row in `tenant_members` for the tenant; best-effort per member (one member's revocation failure doesn't abort the cascade).
     - `getLatestRevokedAt(userId) → number | null` (unix seconds).
     - `isTokenRevoked(userId, iatUnixSeconds) → boolean` — the hot-path check. Fails CLOSED on DB errors (returns `true`) so a broken ledger doesn't silently admit requests.
     - `listRevocationsForUser(userId, limit?)` for audit UIs.

     **Cascade wired into `suspendTenant`** — when a tenant's status transitions to `suspended`, `revokeSessionsForTenant` is called with `reason='tenant.suspend'`. Any JWT whose `iat` is earlier than the resulting ledger row returns **401 `SESSION_REVOKED`** on its next authed request. Cascade failure is non-fatal — the tenant-status transition always completes (the context guard still 403s workspace calls, so the user isn't silently admitted; they just don't get the forced invalidation on existing tokens).

     **`authMiddleware` gate** — after the existing user-status check, the middleware calls `isTokenRevoked(payload.userId, payload.iat)`. On true → 401 with error code `SESSION_REVOKED` (distinct from the generic `UNAUTHORIZED`) so iOS can show bespoke copy ("Your workspace was suspended") rather than generic re-auth. One indexed SELECT per authed request — cheap at current scale; LRU cache can be added later if needed.

     **Time handling** — all comparisons are SQLite-side via `strftime('%s', revoked_at)`. Avoids the ISO-text-vs-unix-seconds parsing ambiguity we've seen bite other services. JWT `iat` (issued-at, unix seconds from jsonwebtoken) is compared as a number against the SQLite-computed revocation epoch.

     **Tests — 21 new pins across 2 files:**
     - `__tests__/services/session-revocation-service.test.ts` (14): ledger writes (reason + actor + details), bad-input error codes, append-only ordering, cascade fans out to tenant_members, no-members returns [], bad-tenant-id error, isTokenRevoked (no-revocation / iat-earlier / iat-later / malformed-iat / only-latest-matters), **suspendTenant integration** (cascade writes revocations; cascade failure is non-fatal — if the table is dropped, suspend still succeeds; activateTenant does NOT issue revocations).
     - `__tests__/api/auth-middleware-session-revocation.test.ts` (7): structural pins on the middleware branch — JWT payload type includes iat, revocation check wired via require, 401 SESSION_REVOKED (not UNAUTHORIZED), fail-closed catch, check order (user-status first, revocation second), operator warn-log includes userId + iat. Full runtime exercise via integration test in session-revocation-service.test.ts — vitest can't easily mock the middleware's inline `require()` for a direct unit test, so we pin the WIRING here and exercise the BEHAVIOR there.
