...
| File                                        | Delta                                                                    |
|---------------------------------------------|--------------------------------------------------------------------------|
| `src/api/portal-workspace-router.ts`        | +186 (console/home) + ~145 (OI-DATA-002) + ~120 (OI-DATA-005) + ~95 (OI-DATA-003) + ~15 (OI-DATA-003a) + ~25 (OI-DATA-003b): `GET/PUT /workspace/skills/:skillId/config` routes + `content.voice.guidelines` + `secretary.routines.set` + `training.goals.set` dependencies all wired into the Home evaluator. |
...
| `src/portal/user-console.html`              | +~340 (OI-USR-404) + ~150 (OI-DATA-002) + ~160 (OI-DATA-005) + ~155 (OI-USR-407) + ~350 (OI-UX-101) + ~220 (OI-DATA-003) + ~200 (OI-USR-405) + ~215 (OI-DATA-003a) + ~215 (OI-DATA-003b): wizard + Channels + Activity + Profile editor + global search + Content + ref-to-skill tags + Secretary editor + **Training Configuration editor** (6 fields: goals / equipment / constraints / days-policy / recovery-priority / extra_notes). |
