...
| `src/portal/user-console.html`              | +~340 (OI-USR-404) + ~150 (OI-DATA-002) + ~160 (OI-DATA-005) + ~155 (OI-USR-407) + ~350 (OI-UX-101) + ~220 (OI-DATA-003) + ~200 (OI-USR-405) + ~215 (OI-DATA-003a) + ~215 (OI-DATA-003b) + ~215 (OI-DATA-003c) + ~215 (OI-DATA-003d) + ~150 (OI-USR-405a) + ~162 (OI-UX-102): wizard + Channels + Activity + Profile editor + global search + Content + ref-to-skill tags + Secretary + Training + Finance + Cooking Configuration editor + ref-to-skill edit-mode chips + **tag autocomplete on Add forms** (`collectTagPool` + `splitTagInput` + `rankTagSuggestions` + `initTagAutocomplete` + `initAllTagAutocomplete` + ~22 lines CSS for `.tag-ac-popover` / `.tag-ac-item`). **All 5 skill pages ship real Configuration editors, every reference row supports inline skill-tag edit, AND every Add-form tag input autocompletes against the existing vocabulary.** |
