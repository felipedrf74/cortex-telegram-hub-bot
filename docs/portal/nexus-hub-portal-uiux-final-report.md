...
| File                                        | Delta                                                                    |
|---------------------------------------------|--------------------------------------------------------------------------|
| `src/api/portal-workspace-router.ts`        | +186 (console/home) + ~145 (OI-DATA-002) + ~120 (OI-DATA-005) + ~95 (OI-DATA-003) + ~15 (OI-DATA-003a) + ~25 (OI-DATA-003b) + ~25 (OI-DATA-003c): `GET/PUT /workspace/skills/:skillId/config` routes + 4 skill-config Home dependencies (content.voice.guidelines, secretary.routines.set, training.goals.set, finance.budget.set). |
...
| `src/portal/user-console.html`              | +~340 (OI-USR-404) + ~150 (OI-DATA-002) + ~160 (OI-DATA-005) + ~155 (OI-USR-407) + ~350 (OI-UX-101) + ~220 (OI-DATA-003) + ~200 (OI-USR-405) + ~215 (OI-DATA-003a) + ~215 (OI-DATA-003b) + ~215 (OI-DATA-003c): wizard + Channels + Activity + Profile editor + global search + Content + ref-to-skill tags + Secretary + Training + **Finance Configuration editor** (6 fields: budget_monthly / saving_goals / affordability_rules / primary_currency / decision_style / extra_notes). |
...
- ~~**OI-DATA-003c** Finance skill config schema~~ **DONE 2026-04-23** — 6-field schema (budget_monthly / saving_goals / affordability_rules / primary_currency / decision_style / extra_notes) + real editor UI + new Home dependency `finance.budget.set` that auto-heals. 4 of 5 skills now have real Configuration tabs — Cooking (OI-DATA-003d) is the last.
