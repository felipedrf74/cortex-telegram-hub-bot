...
| `src/portal/admin-console.html`             | +~420 (OI-ADM-301 + OI-ADM-303) + ~380 (OI-UX-101a) + ~50 (OI-UX-104): tenant detail drawer + filtered audit viewer + global spotlight search with tenant-drawer deep-link + state caching refactor (loadTenants/loadPlatformAdmins/paintOverview capture into state.*) + LABELS map + labelFor helper (shared shape with user-console). |
| `src/portal/user-console.html`              | +~340 (OI-USR-404) + ~150 (OI-DATA-002) + ~160 (OI-DATA-005) + ~155 (OI-USR-407) + ~350 (OI-UX-101) + ~220 (OI-DATA-003) + ~200 (OI-USR-405) + ~215 (OI-DATA-003a) + ~215 (OI-DATA-003b) + ~215 (OI-DATA-003c) + ~215 (OI-DATA-003d) + ~150 (OI-USR-405a) + ~162 (OI-UX-102) + ~48 (OI-UX-104) + ~155 (OI-UX-105) + ~217 (OI-UX-103) + ~50 (OI-USR-406): wizard + Channels + Activity + Profile editor + global search + Content + ref-to-skill tags + Secretary + Training + Finance + Cooking Configuration editor + ref-to-skill edit-mode chips + tag autocomplete + LABELS map + keyboard shortcuts + bulk actions + **invite expiry countdown** (`formatCountdown` + `humanizeDuration` + 30s tick + Expires column + 5-state CSS). **All 5 skill pages ship real Configuration editors, every reference row supports inline skill-tag edit, every Add-form tag input autocompletes, every wire enum renders as a human label, power users can navigate via `g h/b/c/l/n/i/d/a/t/p`, every reference page supports multi-row bulk operations, AND invite timestamps show as relative countdowns with urgency tints.** |
