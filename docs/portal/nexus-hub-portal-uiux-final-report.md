...
| `src/portal/server.ts`                      | +33 lines: route aliases for `/admin-console`, `/console`, `/user-console`. |
...
- **OI-NAV-203** Wire `/invite/accept?code=` landing page so invite links resolve.
...
1. **OI-NAV-203 — invite-accept landing page.** Unblocks the invite flow end-to-end. Today the invite link generated by Team → Invite 404s when clicked. ~1 h of work.
2. **OI-ADM-301 — tenant detail drawer.** Biggest Admin-Console gap; endpoints exist (`/owner/tenants/:id`, `/owner/tenants/:id/members`), just need wiring. ~3 h.
