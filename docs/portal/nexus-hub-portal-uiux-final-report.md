...
| `src/portal/user-console.html`              | +~340 (OI-USR-404) + ~150 (OI-DATA-002) + ~160 (OI-DATA-005) + ~155 (OI-USR-407) + ~350 (OI-UX-101) + ~220 (OI-DATA-003) + ~200 (OI-USR-405) + ~215 (OI-DATA-003a) + ~215 (OI-DATA-003b) + ~215 (OI-DATA-003c) + ~215 (OI-DATA-003d) + ~150 (OI-USR-405a): wizard + Channels + Activity + Profile editor + global search + Content + ref-to-skill tags + Secretary + Training + Finance + Cooking Configuration editor + **ref-to-skill edit-mode chips** (`setSkillTagOnRef` helper + `renderSkillBadgesEditable` + ~60 lines CSS for `.skill-chip-editable` / `.skill-chip-x` / `.skill-chip-add` / `.skill-add-popover` / `.skill-add-menu`). **All 5 skill pages ship real Configuration editors AND every reference row supports inline skill-tag edit.** |
