...
| File                                        | Delta                                                                    |
|---------------------------------------------|--------------------------------------------------------------------------|
| `src/api/portal-workspace-router.ts`        | +186 (console/home) + ~145 (OI-DATA-002) + ~120 (OI-DATA-005) + ~95 (OI-DATA-003) + ~15 (OI-DATA-003a): `GET/PUT /workspace/skills/:skillId/config` routes + `content.voice.guidelines` and `secretary.routines.set` dependencies wired into the Home evaluator. |
...
| `src/portal/user-console.html`              | +~340 (OI-USR-404) + ~150 (OI-DATA-002) + ~160 (OI-DATA-005) + ~155 (OI-USR-407) + ~350 (OI-UX-101) + ~220 (OI-DATA-003) + ~200 (OI-USR-405) + ~215 (OI-DATA-003a): wizard + Channels + Activity + Profile editor + global search + Content + ref-to-skill tags + **Secretary Configuration editor** (6 fields mirroring Content). |
...
- ~~**OI-DATA-003a** Secretary skill config schema~~ **DONE 2026-04-23** — 6-field schema (daily_routines / priority_rules / focus_block_policy / primary_calendar / interruption_tolerance / extra_notes) + real editor UI + new Home dependency `secretary.routines.set` that auto-heals. Mirrors the OI-DATA-003 pattern; remaining skills (Training/Finance/Cooking) tracked as OI-DATA-003b..d.
