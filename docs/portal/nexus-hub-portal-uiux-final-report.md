...
| `__tests__/services/tenant-skill-config-secretary.test.ts`                | 5 KB         | **OI-DATA-003a** — 12 service pins for the Secretary schema: 6-field round-trip, 3 enum validators, 3 length caps, empty-string→null, unknown-field 400 with 6-field allowed list, diff patches |
| `__tests__/api/portal-workspace-secretary-config-routes.test.ts`          | 6 KB         | **OI-DATA-003a** — 5 route pins: GET returns 6 defaults + schemaKeys, PUT wrong-skill field rejection, Home dep `secretary.routines.set` flips missing→ready→missing on set/clear, cost-privacy invariant |
| `__tests__/portal/user-console-secretary-config.test.ts`                  | 4 KB         | **OI-DATA-003a** — 18 UI pins: legacy empty-state gone, 6 input ids, 3 enum-option presence tests, Save/Revert/dirty wiring, diff-only PUT, post-save re-baseline, loadHome refresh, non-admin view-only, Content editor not regressed |
| `__tests__/services/tenant-skill-config-training.test.ts`                 | 5 KB         | **OI-DATA-003b** — 12 service pins: 6-field round-trip, 2 enum validators (days × 5, recovery × 3), 4 length caps (all 2000), empty-string→null, unknown-field 400 with 6-field allowed list, diff patches |
| `__tests__/api/portal-workspace-training-config-routes.test.ts`           | 6 KB         | **OI-DATA-003b** — 5 route pins: GET returns 6 defaults + schemaKeys, PUT wrong-skill field rejection, Home dep `training.goals.set` flips missing→ready→missing on set/clear, cost-privacy invariant |
| `__tests__/portal/user-console-training-config.test.ts`                   | 4 KB         | **OI-DATA-003b** — 18 UI pins: same pattern as Secretary, plus enum-defaults-match-service pin (four_days + balanced), plus Content + Secretary editors not regressed |
| `__tests__/services/tenant-skill-config-finance.test.ts`                  | 5 KB         | **OI-DATA-003c** — 12 service pins: 6-field round-trip, 2 enum validators (currency × 6, decision_style × 3), 3 length caps, empty→null, unknown-field 400, diff patches |
| `__tests__/api/portal-workspace-finance-config-routes.test.ts`            | 6 KB         | **OI-DATA-003c** — 5 route pins: GET defaults + schemaKeys, PUT valid body, wrong-skill field rejection, Home dep flip, cost-privacy (4-decimal marker) |
| `__tests__/portal/user-console-finance-config.test.ts`                    | 4 KB         | **OI-DATA-003c** — 18 UI pins: same pattern as Training, plus enum-defaults-match-service (USD + balanced), plus Content/Secretary/Training editors not regressed |
| `__tests__/services/tenant-skill-config-cooking.test.ts`                  | 5 KB         | **OI-DATA-003d** — 12 service pins: 6-field round-trip, 2 enum validators (serving_size × 5, meal_cost_ceiling × 4), 3 length caps, empty→null, unknown-field 400, diff patches |
| `__tests__/api/portal-workspace-cooking-config-routes.test.ts`            | 6 KB         | **OI-DATA-003d** — 5 route pins: GET defaults + schemaKeys, PUT valid body, wrong-skill field rejection, Home dep flip, **safety-gate pin** (preferences alone does NOT flip restrictions.set to ready) |
| `__tests__/portal/user-console-cooking-config.test.ts`                    | 4 KB         | **OI-DATA-003d** — 19 UI pins: same pattern as Finance, plus HARD-constraints label pin, plus 5-editor canary pin (Content + Secretary + Training + Finance + Cooking all wired correctly) |
| `__tests__/portal/user-console-ref-skill-edit.test.ts`                    | 8 KB         | **OI-USR-405a** — 30 pins for edit-mode: `SKILL_EDIT_KINDS` lookup shape, `setSkillTagOnRef` (parseSkillTags read, add dedup, remove filter, non-skill tags preserved, PATCH shape, error toast), `renderSkillBadgesEditable` (chip + × + popover only when available.length > 0, popover auto-close, escape gating), 4 row renderers wired, 7 CSS pins (cursor, danger hover, dashed border, marker-strip, absolute menu, z-index), regression pin that read-only `renderSkillBadges` still exists for Insights/detail views. |
| `__tests__/portal/user-console-tag-autocomplete.test.ts`                  | 8 KB         | **OI-UX-102** — 31 pins for tag autocomplete: `collectTagPool` walks all 4 state arrays + skill-tag exclusion + defensive guards (Array.isArray, typeof string), `splitTagInput` (lastIndexOf split, empty/first-comma/multi-comma), `rankTagSuggestions` (prefix > substring ordering, already-used filter, empty-prefix show-all path, count+alpha tiebreak, max 8, case-insensitive), `initTagAutocomplete` (dataset-guard idempotency, body-append, getBoundingClientRect fixed positioning, listeners, mousedown-not-click, 150ms blur delay, Escape handler, esc() on innerHTML), 3 inputs bound (no Channels), single shared boot `setTimeout`, CSS pins (position:fixed, z-index:30, max-height 200 + overflow-y auto, flex-between row), regression pin that `class="grow"` survived on noteTags. |
| `__tests__/portal/portal-label-map.test.ts`                               | 8 KB         | **OI-UX-104** — 35 pins for the wire-enum → human label map: both consoles declare identical LABELS (6 kinds × all value pairs pinned: bookStatus × 4, channelKind × 7, role × 5, inviteStatus × 4, tenantPlan × 3, tenantStatus × 3), `labelFor` signature + null/undefined/empty handling + raw-fallback, 7 user-console display sites + 4 admin-console display sites go through labelFor, 4 absence pins verify old `esc(x.role)` / `esc(i.role)` / `esc(i.status)` / `esc(ch.kind)` leaks gone, wire/display split regression (option values stay snake_case so POST/PATCH validate). |
| `__tests__/portal/user-console-keyboard-shortcuts.test.ts`                | 9 KB         | **OI-UX-105** — 29 pins for keyboard shortcuts: SHORTCUT_G_MAP (10 key→page pairs), cross-check that every key in the map has a `<kbd>g</kbd><kbd>X</kbd>` row in the help modal (catches drift), `isTypingInInput` definition + input-guard-first contract, modifier-key bailout, kbPending state machine (set/clear helpers, 1500ms expiry, timer-replace on set, one-shot consumption via `clearPending` at dispatch, `toLowerCase()` on 2nd char), / and ? handlers, Esc-only-when-open guard, #kbShortcutsModal structure (role=dialog + 3 sections + Close button + footer disclaimer), CSS pins (position:fixed + z-index:72 one above search at 71, 2-column grid body, monospace kbd), open/close helpers exposed on window, regression pin that Cmd+K listener + searchOpen/Close survived. |
| `__tests__/portal/user-console-bulk-actions.test.ts`                      | 11 KB        | **OI-UX-103** — 41 pins for bulk actions: `state.bulkSel` per-kind Set shape, `BULK_KINDS` lookup (4 kinds × path + reload + findById + render), `bulkToggle` / `bulkToggleAll` (visible-only + progressive-all-off affordance + re-render-on-toggle), `bulkDelete` (sequential-not-parallel, `confirm()` gate, partial-success toast shape, DELETE shape, clear-then-reload), `bulkAddSkill` (idempotent skip + humanised skill-label toast + non-skill-tag preservation + PATCH shape + sequential-not-parallel), `renderBulkToolbar` (zero-state returns empty string, full markup, role=toolbar + aria-label, ≥6 `esc(kind)` uses for defense-in-depth), 4 renderers wired (Books/Links/Channels checkbox column in thead + bulkToggleAll with `visibleIds.every` shape + per-row checkbox + row-selected class; Notes leading flex-inline "Select all" row), CSS anchors (accent-subtle tint on toolbar + row-selected, flex-gap layout, 32px check column), regression pins that inline Remove buttons + tag autocomplete + skill-badges-editable all survive. |
| `__tests__/portal/user-console-invite-countdown.test.ts`                  | 10 KB        | **OI-USR-406** — 26 pins for invite expiry countdown: **actual behavior tests** of `formatCountdown` + `humanizeDuration` extracted from the HTML via `new Function(...)` and executed against `vi.setSystemTime()`-frozen clocks (5 urgency buckets × boundary cases: 24h↔fresh/soon boundary, 1h↔soon/expiring, sub-minute 1m clamp, exactly-0 = expired), plus structural pins for the Expires column header, row-level countdown span (class + data-expires + title + iso-fallback), 30s tick via setInterval, CSS pins (tabular-nums, 5 kind classes, orange expiring + danger expired colors, italic none), and regression pins that OI-UX-104 `labelFor` calls on role + inviteStatus survived. |
| `__tests__/portal/portal-responsive-collapse.test.ts`                     | 10 KB        | **OI-UX-106** — 47 pins for <768px responsive collapse (applies to BOTH consoles via parameterised test suite): `#mobileNavToggle` button shape + aria attributes + display:none default, `@media (max-width: 768px)` block existence, shell grid collapses to single column, sidebar becomes fixed-position slide-in drawer with `min(280px, 80vw)` + translateX(-100%) default + `.mobile-open` applies translateX(0), **backdrop pseudo-element uses `pointer-events: none`** so document click-outside handler captures taps, app-bar divider + scope-pill + search-label + search-kbd hidden, user-console extras (tighter page padding, `.page table.data { overflow-x: auto }` horizontal-scroll wrapping, bulk-toolbar flex-wrap, kb-modal-body single-column stack, modal top:5vh for keyboard-open room), mobile-nav JS helpers (toggleMobileNav + closeMobileNav + window exposure + aria-expanded sync), `showPage` calls `closeMobileNav()` so every navigation path auto-closes, document click-outside handler with inside-drawer + inside-hamburger guards, regression pins that default desktop grid stays `260px 1fr` and default sidebar has no `position: fixed` outside the media query. |
...
| File                                        | Delta                                                                    |
|---------------------------------------------|--------------------------------------------------------------------------|
| `src/api/portal-workspace-router.ts`        | +186 (console/home) + ~145 (OI-DATA-002) + ~120 (OI-DATA-005) + ~95 (OI-DATA-003) + ~15 (OI-DATA-003a) + ~25 (OI-DATA-003b) + ~25 (OI-DATA-003c) + ~25 (OI-DATA-003d): `GET/PUT /workspace/skills/:skillId/config` routes + **5 skill-config Home dependencies**, one per skill: `content.voice.guidelines`, `secretary.routines.set`, `training.goals.set`, `finance.budget.set`, `cooking.restrictions.set`. |
...
| `src/portal/admin-console.html`             | +~420 (OI-ADM-301 + OI-ADM-303) + ~380 (OI-UX-101a) + ~50 (OI-UX-104) + ~75 (OI-UX-106): tenant detail drawer + filtered audit viewer + global spotlight search with tenant-drawer deep-link + state caching refactor + LABELS map + labelFor helper + **responsive collapse for <768px** (hamburger button + `toggleMobileNav`/`closeMobileNav` + `showPage` auto-close + click-outside listener + `@media` block for sidebar drawer). |
| `src/portal/user-console.html`              | +~340 (OI-USR-404) + ~150 (OI-DATA-002) + ~160 (OI-DATA-005) + ~155 (OI-USR-407) + ~350 (OI-UX-101) + ~220 (OI-DATA-003) + ~200 (OI-USR-405) + ~215 (OI-DATA-003a) + ~215 (OI-DATA-003b) + ~215 (OI-DATA-003c) + ~215 (OI-DATA-003d) + ~150 (OI-USR-405a) + ~162 (OI-UX-102) + ~48 (OI-UX-104) + ~155 (OI-UX-105) + ~217 (OI-UX-103) + ~50 (OI-USR-406) + ~96 (OI-UX-106): wizard + Channels + Activity + Profile editor + global search + Content + ref-to-skill tags + Secretary + Training + Finance + Cooking Configuration editor + ref-to-skill edit-mode chips + tag autocomplete + LABELS map + keyboard shortcuts + bulk actions + invite expiry countdown + **responsive collapse for <768px** (hamburger + slide-in drawer + click-outside + tight app-bar + horizontal-scroll tables + single-column help modal). **All 5 skill pages ship real Configuration editors, every reference row supports inline skill-tag edit, every Add-form tag input autocompletes, every wire enum renders as a human label, power users can navigate via `g h/b/c/l/n/i/d/a/t/p`, every reference page supports multi-row bulk operations, invite timestamps show as relative countdowns with urgency tints, AND the whole shell collapses gracefully to a drawer-based mobile layout.** |
...
- ~~**OI-DATA-003a** Secretary skill config schema~~ **DONE 2026-04-23** — 6-field schema (daily_routines / priority_rules / focus_block_policy / primary_calendar / interruption_tolerance / extra_notes) + real editor UI + new Home dependency `secretary.routines.set` that auto-heals. Mirrors the OI-DATA-003 pattern.
- ~~**OI-DATA-003b** Training skill config schema~~ **DONE 2026-04-23** — 6-field schema (goals / equipment_available / constraints_and_injuries / preferred_training_days / recovery_priority / extra_notes) + real editor UI + new Home dependency `training.goals.set` that auto-heals. Forward-compatible with the Phase 1 triathlon→gym/running/cycle/swim sub-skill split.
- ~~**OI-DATA-003c** Finance skill config schema~~ **DONE 2026-04-23** — 6-field schema (budget_monthly / saving_goals / affordability_rules / primary_currency / decision_style / extra_notes) + real editor UI + new Home dependency `finance.budget.set` that auto-heals.
- ~~**OI-DATA-003d** Cooking skill config schema~~ **DONE 2026-04-23 — 5/5 SKILLS COMPLETE.** 6-field schema (dietary_restrictions / preferences / kitchen_inventory / serving_size / meal_cost_ceiling / extra_notes) + real editor UI + Home dependency `cooking.restrictions.set` gated on HARD constraints (allergy-safe, not preferences). All 5 Configuration tabs are now real editors backed by the tenant_skill_config infrastructure.
- ~~**OI-USR-405a** Reference-to-skill edit mode~~ **DONE 2026-04-23** — every reference row (books / channels / links / notes) now has an inline chip widget: assigned-skill chips are click-to-remove (× glyph, red hover); a dashed "+ Skill" chip opens a `<details>`/`<summary>` popover listing only unassigned skills; each click is one PATCH. Home reloads after every mutation so dependency badges stay in sync. Zero backend change — rides on the existing PATCH routes. 30 new structural pins; 4 existing OI-USR-405 render-call pins updated to the editable-renderer form.
- ~~**OI-UX-102** Reference tag autocomplete~~ **DONE 2026-04-23** — free-form tag inputs on Books / Links / Notes Add forms now show a ranked dropdown of existing tags as the user types. Pool is recomputed per keystroke from the 4 state arrays (skill:* tags excluded — skill-picker owns those). Prefix matches rank above substring matches; count DESC with alphabetical tiebreak; max 8 suggestions. Selection appends `tag, ` so tags can be chained. Popover is `position:fixed` appended to `<body>` (avoids breaking the `.grow` flex-grow on noteTags). `mousedown`-not-`click` wins the input-blur race. 31 new structural pins. Zero backend change.
- ~~**OI-UX-104** Wire-enum → human label map~~ **DONE 2026-04-23** — 11 raw-enum display sites (7 in user-console, 4 in admin-console) no longer leak snake_case wire values. A central `LABELS` map + `labelFor(kind, value)` helper lives identically in both consoles and covers 6 enum kinds (bookStatus, channelKind, role, inviteStatus, tenantPlan, tenantStatus). Raw fallback preserves forward-compat: a new server-side enum shows as the raw value (visible bug > silent render gap). Wire/display split preserved — form `<option value="...">` attributes still carry wire values; only display pills are humanised. 35 new structural pins.
- ~~**OI-UX-105** Keyboard shortcuts~~ **DONE 2026-04-23** — User Console now supports g-prefix navigation (10 bindings — `g h/i/d/a/t/p` for nav pages + `g b/c/l/n` for Reference Center), `/` opens search alongside Cmd+K, and `?` opens a 2-column help modal. Four correctness guards: input-focus guard bails the handler when the target is any editable element (typing "g" in a note body stays literal); modifier-key bailout preserves browser shortcuts; g-prefix expires after 1500ms (Vim convention); `preventDefault()` only fires on keys actually consumed. Search-modal footer now lists `? shortcuts` so users discover the full binding set. 29 new structural pins; zero backend change.
- ~~**OI-UX-103** Bulk actions on reference tables~~ **DONE 2026-04-23** — every reference page (Books / Channels / Links / Notes) supports row selection + two bulk operations: delete (guarded by `confirm()`, sequential, partial-success reporting) and add-skill (idempotent — rows that already carry the skill are counted + skipped). Checkbox column on tables; a leading "Select all" flex-inline row on the Notes div-list. Bulk toolbar (accent-tinted) appears above the list when ≥1 row is selected and shows "N selected" + skill-selector + Apply + Delete + Clear. "Select all" acts on visible rows only (respects active filters). Selection state is Set<id> per kind in `state.bulkSel`; operations are sequential so the rate limiter stays predictable and partial failures can be reported honestly. Zero backend change — rides on existing single-row PATCH + DELETE routes. 41 new structural pins.
- ~~**OI-USR-406** In-UI invite expiry countdown~~ **DONE 2026-04-23** — Team page's Invites table gains an "Expires" column with a relative countdown (5 urgency tints: `none`/`fresh`/`soon`/`expiring`/`expired`). Pure helper `formatCountdown(iso) → { label, kind }` at day/hour/minute resolution with a sub-minute 1m clamp; 30s setInterval tick updates `textContent` + `className` only (no row replacement, so in-flight Copy/Revoke clicks survive). `font-variant-numeric: tabular-nums` prevents digit jitter as numbers change. 26 new pins including **executed-behavior tests** against frozen clocks via `new Function(...)` extraction — first file on the branch to test actual helper outputs rather than just structural shape. Zero backend change.
- ~~**OI-UX-106** Responsive collapse for < 768 px~~ **DONE 2026-04-23** — both consoles (`user-console.html` + `admin-console.html`) now ship a `@media (max-width: 768px)` block. Sidebar becomes a slide-in drawer (`position: fixed`, `width: min(280px, 80vw)`, `transform: translateX(-100%)` default, `.mobile-open` applies `translateX(0)`). A hamburger button (`#mobileNavToggle`) shows only at mobile widths + tracks state via `aria-expanded`. Backdrop pseudo-element uses `pointer-events: none` so taps pass through to a document click-outside handler. `showPage()` calls `closeMobileNav()` at the tail — every navigation path (nav click, search pick, keyboard shortcut, deep link) auto-closes the drawer without parallel handlers. user-console additionally tightens page padding, wraps dense reference tables in `overflow-x: auto` (horizontal scroll preserves column mental map; no card-stacking), stacks the shortcuts-help modal body to single column, and flex-wraps the bulk toolbar. 47 new pins. Zero backend change.
