...
| `src/portal/user-console.html`              | +~340 (OI-USR-404) + ~150 (OI-DATA-002) + ~160 (OI-DATA-005) + ~155 (OI-USR-407) + ~350 (OI-UX-101) + ~220 (OI-DATA-003) + ~200 (OI-USR-405) + ~215 (OI-DATA-003a) + ~215 (OI-DATA-003b) + ~215 (OI-DATA-003c) + ~215 (OI-DATA-003d) + ~150 (OI-USR-405a) + ~162 (OI-UX-102) + ~48 (OI-UX-104): wizard + Channels + Activity + Profile editor + global search + Content + ref-to-skill tags + Secretary + Training + Finance + Cooking Configuration editor + ref-to-skill edit-mode chips + tag autocomplete on Add forms + **LABELS map + labelFor helper** (shared with admin-console; covers bookStatus / channelKind / role / inviteStatus / tenantPlan / tenantStatus). **Every user-facing pill, chip, and subtitle now renders wire enums as human labels.** |
