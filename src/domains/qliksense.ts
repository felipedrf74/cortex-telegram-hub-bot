...
const DOMAIN: DomainName = 'qliksense';

async function buildStateContext(): Promise<string> {
  const parts: string[] = [];
  parts.push(`Today: ${now().toFormat('cccc, LLLL dd yyyy, HH:mm')} (Europe/Lisbon)`);

  const todos = listTodos({ domain: 'qliksense', status: 'pending' });
  if (todos.length > 0) {
    parts.push(`\nQlik Sense to-dos (${todos.length}):`);
...
export async function handleQlikSense(message: string): Promise<DomainResponse> {
