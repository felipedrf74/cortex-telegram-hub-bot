...
export function createAdminPreBodyGuard(
  ...
  router.use((req: Request, res: Response, next: NextFunction) => {
    ...
    // A full map may only contain expired buckets between periodic prunes. Do
    // one deterministic prune before assigning a new client to overflow.
    if (!requestLog.has(clientIp) && requestLog.size >= maxTrackedIps) {
      pruneStaleRequestBuckets(requestLog, currentTime, windowMs);
    }
