...
async function load() {
  ...
  try {
    ...
    const chips = Object.entries(grouped).map(([key, rows]) =>
      '<span class="u-mr-6 u-mb-6 badge ' + (rows.length ? 'badge-accent' : 'badge-neutral') + '">' + P.esc(key) + ' · ' + rows.length + '</span>').join('');

    const rows = items.map((i) => '<tr>' +
      '<td><span class="badge badge-neutral">' + P.esc(i.sourceSkill || 'system') + '</span></td>' +
      '<td><div class="u-fw-600">' + P.esc(i.title) + '</div>' +
        '<div class="u-fs-12 u-lh-1-4 text-muted">' + P.esc(i.summary || '') + '</div>' +
        (i.blockedByDecisionIds && i.blockedByDecisionIds.length ? '<div class="u-mt-6 badge badge-warning">blocked by ' + i.blockedByDecisionIds.length + '</div>' : '') +
      '</td>' +
      ...
      '<td>' + ((i.actions || []).slice(0, 2).map((a) =>
        '<button class="u-mr-4 btn btn-xs" data-op="decision-action" data-decision="' + P.esc(String(i.decisionId)) + '" data-action="' + P.esc(String(a.id)) + '">' + P.esc(a.label || a.id) + '</button>').join('') || '<span class="text-muted">—</span>') + '</td>' +
      '<td class="u-fs-11 mono">' + P.esc(i.createdAt || '—') + '</td>' +
    '</tr>').join('');
    el('notification-center-content').innerHTML =
      '<div class="u-mb-space-3">' + chips + '</div>' +
      '<div class="u-ovx-auto"><table class="data-table"><thead><tr><th>Skill</th><th>Safe preview</th><th>Type</th><th>Urgency</th><th>Status</th><th>Actions</th><th>Created</th></tr></thead>' +
    ...
    el('notification-reliability-content').innerHTML =
      '<div class="u-ovx-auto"><table class="data-table"><thead><tr><th>Area</th><th>Metric</th><th>Value</th><th>Metric</th><th>Value</th></tr></thead><tbody>' + reliabilityRows + '</tbody></table></div>' +
      '<div class="u-ovx-auto u-mt-space-3"><table class="data-table"><thead><tr><th>Skill</th><th>Type</th><th>Recipe</th><th>Gated</th><th>Deduped</th><th>Superseded</th><th>Unsupported</th><th>Failed</th><th>Dead links</th></tr></thead>' +
      '<tbody>' + (topicRows || '<tr><td colspan="9"><div class="empty">No topic reliability events</div></td></tr>') + '</tbody></table></div>' +
      '<div class="u-mt-space-2 u-fs-12 text-muted">Generated ' + P.esc(reliability.generatedAt || '—') + '</div>';

    const onOff = (v) => (v ? 'on' : 'off');
    el('notification-preferences-content').innerHTML =
      '<div class="u-ovx-auto"><table class="data-table"><thead><tr><th>User</th><th>Push</th><th>Urgent push</th><th>Time-sensitive</th><th>Auto-hide resolved</th><th>Digest</th><th>Updated</th></tr></thead>' +
      ...
      '<td class="u-fs-11 mono">' + P.esc(profile.updatedAt || '—') + '</td></tr></tbody></table></div>';
  } catch (err) {
...
function card(title, subtitle, id, loading, extraClass) {
  return '<div class="card' + (extraClass ? ' ' + extraClass : '') + '"><div class="card-header"><div><div class="card-title">' + title + '</div><div class="card-subtitle">' + subtitle + '</div></div></div><div id="' + id + '"><div class="empty">' + loading + '</div></div></div>';
}
...
function mount(container) {
  ...
  root.innerHTML =
    ...
    '<div class="u-mb-space-4 kpi-grid">' + kpi('Unread', 'notification-kpi-unread', 'active center items') + kpi('Needs decision', 'notification-kpi-decision', 'approval or conflict') + kpi('Preferences', 'notification-kpi-profiles', 'scoped profiles') + '</div>' +
    ...
    card('Notification reliability', 'Dedupe, digest, push, badge drift, and read-state telemetry for the selected scope', 'notification-reliability-content', 'Loading reliability…', 'mt-4') +
    card('Notification preferences', 'Channel and digest settings by scoped user profile', 'notification-preferences-content', 'Loading preferences…', 'mt-4');
