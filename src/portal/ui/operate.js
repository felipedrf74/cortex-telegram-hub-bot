...
function genericTable(rows, emptyText, maxCols) {
  ...
  return '<div style="overflow-x:auto"><table class="data-table dense"><thead><tr>' +
...
function renderFlags() {
  ...
  box.innerHTML = areas.map((area) =>
    '<div class="mt-4"><div class="card-subtitle" style="text-transform:uppercase;letter-spacing:.04em">' + P.esc(area) + '</div>' +
...
async function loadProviders() {
  ...
  try {
    ...
    box.innerHTML = '<div class="grid grid-cols-2">' + providers.map((p) =>
      ...
      '<div class="text-muted" style="font-size:12px;margin-top:6px">' + p.probes + ' probes · ' + Math.round(p.failureRate * 100) + '% failed · avg ' + (p.avgLatencyMs != null ? p.avgLatencyMs + 'ms' : '—') + ' · p95 ' + (p.p95LatencyMs != null ? p.p95LatencyMs + 'ms' : '—') + '</div>' +
      (p.lastError ? '<div class="mono text-muted" style="font-size:11px;margin-top:4px" title="' + P.esc(p.lastError) + '">last error: ' + P.esc(p.lastError.slice(0, 90)) + '</div>' : '') +
...
function chips(map) {
  ...
  return entries.map(([k, v]) => '<span class="badge badge-' + tone(k) + '" style="margin-right:4px">' + P.esc(k) + ' ' + v + '</span>').join('');
}
...
function depthCard(prefix, title, subtitle) {
  return '<div class="card"><div class="card-header"><div class="card-title">' + title + '</div><span class="card-subtitle">' + subtitle + '</span></div>' +
    '<div class="grid grid-cols-4" style="margin-top:var(--space-3)">' +
...
function mount(container) {
  ...
  root.innerHTML =
    ...
    '<div class="section-actions"><span class="text-muted mono" style="font-size:11px" id="q-status"></span> <button class="btn btn-ghost btn-sm" id="op-refresh">Refresh all</button></div></div>' +
    ...
    '<div class="section-actions"><select class="input" id="dl-kind" style="max-width:140px"><option value="jobs">Jobs</option><option value="events">Events</option></select>' +
    '<span class="text-muted mono" style="font-size:11px" id="dl-count"></span></div></div>' +
    '<div style="overflow-x:auto"><table class="data-table dense"><thead><tr><th>Id</th><th>Type</th><th>Tenant / user</th><th class="text-right">Attempts</th><th>Last error</th><th>Failed</th><th></th></tr></thead>' +
    ...
    '<div class="section-actions"><input class="input" id="flags-filter" placeholder="Filter by name, area, env key…" style="max-width:260px">' +
    '<span class="text-muted mono" style="font-size:11px" id="flags-count"></span></div></div>' +
    '<div class="text-muted" style="font-size:12px">● env key set · ○ default. Values are parsed readings; env flags change only through deployment configuration.</div>' +
    ...
    '<div class="section-actions"><select class="input" id="ph-hours" style="max-width:120px"><option value="24">24h</option><option value="72">72h</option><option value="168">7d</option></select></div></div>' +
    ...
    '<div class="section-actions"><select class="input" id="nd-hours" style="max-width:110px"><option value="24">24h</option><option value="72">72h</option><option value="168">7d</option></select>' +
    '<input class="input" id="nd-status" placeholder="status (sent, failed…)" style="max-width:180px">' +
    '<button class="btn btn-ghost btn-sm" id="nd-apply">Apply</button><span class="text-muted mono" style="font-size:11px" id="nd-count"></span></div></div>' +
    '<div id="nd-summary" class="mt-4"></div>' +
    '<div style="overflow-x:auto" class="mt-4"><table class="data-table dense"><thead><tr><th>When</th><th>User</th><th>Channel</th><th>Status</th><th>Code</th><th>Error</th><th>Notification</th></tr></thead>' +
    ...
    '<div class="section-actions"><input class="input" id="wh-owner" placeholder="owner user id (required when operator scopes are set)" style="max-width:320px"><button class="btn btn-ghost btn-sm" id="wh-load">Load</button></div></div>' +
