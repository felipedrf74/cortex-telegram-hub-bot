...
const NL_KEYWORD_ROUTES: { domain: DomainName; pattern: RegExp }[] = [
  ...
  { domain: 'triathlon', pattern: /\b(workout|gym\s+session|running\s+plan|cycling\s+plan|sets?\s*[x×]\s*\d|protein|carnivore|training\s+plan|macros|deload|squat|deadlift|bench\s+press|heart\s+rate|RPE|RIR|tempo\s+run|intervals?|FTP|soreness|recovery\s+day|muscle|hypertrophy|endurance|treino|corrida|pedal(?:ada)?|muscula[çc][aã]o|prote[ií]na|dieta\s+carn[ií]vora|agachamento|supino|levantamento\s+terra|frequ[eê]ncia\s+card[ií]aca|dor\s+muscular|recupera[çc][aã]o|s[eé]ries?\s*[x×]\s*\d)\b/i },
