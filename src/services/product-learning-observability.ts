...
function emptyReadModel(generatedAt: string, tenantId: number | null): ProductLearningObservabilityReadModel {
  return {
    ...
    scope: { tenantId },
    totals: { cases: 0, staleCases: 0, exportEligibleGoldenCases: 0, promotions: 0 },
    lifecycleCounts: zeroLifecycleCounts(),
    transitionCounts: {},
    feedback: { adaptationAccepted: 0, adaptationDismissed: 0, acceptanceRate: null },
    coverage: {
...
export function buildProductLearningObservabilityReadModel(input: {
  ...
  const caseParams = tenantId == null ? [generatedAt] : [generatedAt, tenantId];
  const total = db.prepare(`
    SELECT COUNT(*) AS cases,
           SUM(CASE WHEN lifecycle <> 'retired' AND datetime(expires_at) <= datetime(?) THEN 1 ELSE 0 END) AS staleCases,
           SUM(CASE WHEN lifecycle = 'golden' AND privacy_class <> 'sensitive-no-export' THEN 1 ELSE 0 END) AS exportEligibleGoldenCases
      FROM product_learning_cases
     WHERE owner = 'training'${tenantPredicate}
  `).get(...caseParams) as TotalRow;

  const lifecycleParams = tenantId == null ? [] : [tenantId];
  ...
  for (const row of lifecycleRows) {
    if (LIFECYCLES.includes(row.lifecycle)) lifecycleCounts[row.lifecycle] = Number(row.count);
  }
  ...
  `).all(...lifecycleParams) as TransitionRow[];
  ...
  const adaptationAccepted = categoryByKind.get('adaptation_accepted')!.observedCount;
  const adaptationDismissed = categoryByKind.get('adaptation_rejected')!.observedCount;
  const feedbackTotal = adaptationAccepted + adaptationDismissed;
  const missingCategories = categories.filter((metric) => metric.observedCount === 0).map((metric) => metric.kind);
  return {
    schemaVersion: PRODUCT_LEARNING_OBSERVABILITY_SCHEMA,
    generatedAt,
