...
function buildPlanFromSteps(input: ChatPlannerInput, steps: ChatPlanStep[], routingSignals: string[], confidence: number): ChatActionPlan {
  ...
  return {
    ...
    steps,
    requiresConfirmation: steps.length >= 3 || steps.some((step) => stepRequiresConfirmation(step, { requireSafeWrites })),
...
function shouldRunResolvedMultiStepWithClarification(plan: ChatActionPlan): boolean {
  if (plan.steps.length < 2) return false;
  if (!plan.steps.some((step) => !step.requiredArgsPresent)) return false;
  if (!plan.steps.some((step) => step.requiredArgsPresent)) return false;
  return plan.steps.every((step) => isSafeMixedClarificationRisk(step.risk));
}

function isSafeMixedClarificationRisk(risk: ChatActionRisk): boolean {
  return risk === 'read_only' || risk === 'safe_write' || risk === 'ambiguous';
}
...
export async function executeChatActionPlan(
  ...
  if (hasUnresolvedStep) {
    ...
    if (!shouldRunResolvedMultiStepWithClarification(plan)) {
  ...
  for (const step of plan.steps) {
    ...
    if (executor) {
      results.push(await executeStepWithReliability(runtimeStep, {
        plan,
        input,
        deps,
  ...
  if (needsClarification && results.some((result) => result.status === 'verified_success')) {
    const question = plan.clarificationQuestion || buildTargetedClarificationQuestion(input, plan.steps);
    return buildActionResponse(input, plan, 'partial_success', mixedClarificationCopy(input, question), {
      type: 'chat_action_multi_step_result',
...
function mixedClarificationCopy(input: ChatPlannerInput, question: string): string {
  return input.locale?.startsWith('pt')
    ? `Fiz as partes que estavam claras. Para terminar o resto: ${question}`
    : `I completed the parts that were clear. To finish the rest: ${question}`;
}
