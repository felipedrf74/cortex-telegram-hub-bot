...
function parseCreateTaskWithSubtasksDescriptor(
  ...
  const explicitList = parseExplicitTaskWithSubtasksList(cleaned);
...
function parseExplicitTaskWithSubtasksList(cleaned: string): { title: string; subtasks: string[]; language: string; confidence: number } | null {
  const match = cleaned.match(/^\s*(?:create|add|make|cria|criar|crie|adiciona|adicionar|crear|crea|agrega|agregar|añade|anade|añadir|anadir)\s+(?:a\s+|uma?\s+|una?\s+)?(?:tasks?|todo|to-do|tarefas?|tareas?)\s+(?:called|named|chamada?|chamado?|llamada?|llamado?)\s+(.+?)\s*:\s*([\s\S]+)$/i);
  if (!match) return null;
  const title = normalizeTaskGuidanceTitle(match[1]);
  const subtasks = splitTaskSubtaskItems(match[2] || '');
  if (!title || subtasks.length < 2) return null;
  return { title, subtasks, language: detectTaskLanguage(cleaned), confidence: 0.93 };
}
