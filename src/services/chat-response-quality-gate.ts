...
const SIDE_EFFECT_SUCCESS_VERBS: ReadonlyArray<RegExp> = [
  ...
  /\bi(?:['’]ve| have| just| already)*\s+(?:saved|added|marked|completed|scheduled|sent|deleted|removed|updated|published|posted|uploaded|cancelled|canceled)\b/i,
  // Portuguese
  /(?:^|\s)(?:publiquei|postei|subi|cadastrei|programei|guardei|salvei|adicionei|marquei|conclu[ií]|agendei|enviei|apaguei|removi|cancelei|atualizei)(?=$|\s|[.!?,;:])/i,
  /\benviei (o|a|os|as|isso|para)\b/i,
  /\b(agendei (o|a|os|as)|atualizei (o|a)|adicionei ao calend[áa]rio)\b/i,
  // Spanish
  /(?:^|\s)(?:guardé|a[ñn]ad[ií]|marqué|completé|programé|envié|eliminé|actualicé|cancelé|publiqué|sub[ií])(?=$|\s|[.!?,;:])/i,
];
...
function detectChatResponseQualityIssuesWithSkipInfo(
  ...
  const matchesAnswerOnlySuccessPattern =
    ...
    && claimsSuccess
    && (matchesAppSideEffectClaim || hasConcreteStateSpecifics(unquoted))
