...
function hasForwardActionVerb(text: string): boolean {
  const folded = foldCalendarText(text);
  return /\b(?:create|add|schedule|agenda[r]?|book|delete|remove|cancel|update|change|send|draft|reply|move|reschedule|pay|categorize|snooze|dismiss|choose|pick|notify|alert|reflow|adjust|build|generate|make|set\s+up|block\s+off|rewrite|compose|plan|mark|complete|cria[r]?|adiciona[r]?|marca[r]?|agendar|apaga[r]?|cancela[r]?|atualiza[r]?|muda[r]?|envia[r]?|manda[r]?|mover|remarca[r]?|paga[r]?|classifica[r]?|escolhe[r]?|notifica[r]?|ajusta[r]?|faz(?:er)?|plane[ja]r|reescreve[r]?|criar|crear|a[nñ]ade|programa[r]?|borra[r]?|cambia[r]?|env[ií]a|elige|mueve|reprograma|publica|categoriza|sustituye|substitui|replace|substitute)\b/.test(folded);
}
