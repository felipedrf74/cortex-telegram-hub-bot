...
export function executeContentPipelineStageTransitionStep(
  ...
  try {
    ...
    const row = findScopedPipelineRow(db, input, args, topicTitle);
    if (!row) {
...
function findScopedPipelineRow(
  ...
  if (pipelineId != null) {
    return readScopedPipelineRow(db, input, pipelineId);
  }
  const like = `%${topicTitle.slice(0, 48)}%`;
  const row = db.prepare(`
    SELECT id, topic_title, stage, stage_history, published_url, published_at, youtube_video_id
     ...
     WHERE user_id = ?
       AND tenant_id = ?
       AND COALESCE(scope_status, 'active') = 'active'
       AND (LOWER(topic_title) = LOWER(?) OR LOWER(topic_title) LIKE LOWER(?))
     ORDER BY updated_at DESC, created_at DESC
     LIMIT 1
  `).get(input.userId, input.tenantId, topicTitle, like) as ContentPipelineRow | undefined;
  return row ?? null;
}
