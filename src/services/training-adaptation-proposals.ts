...
async function bindPendingProposalDecision(
  ...
  const created = await createDecisionIntent({
    ...
    idempotencyKey: `training-adaptation:${input.scope.tenantId}:${input.scope.userId}:${row.proposal_id}:${row.option_hash}`,
    channel: 'internal',
    ...
    decisionContext: {
      ...
      contextObservedAt: row.created_at,
      ...
      sourceHealthSnapshot: [{
        source: 'training_adaptation_store', status: 'available', observedAt: row.created_at, staleAfter: row.expires_at,
      }],
      normalizedAction,
    },
  });
