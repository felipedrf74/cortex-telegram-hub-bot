...
const DEFAULT_ALLOWED_DOMAINS: ChatCoreV2Domain[] = [
  'secretary',
  'tasks',
  'training',
  'content',
...
const DEFAULT_SHADOW_CANARY_DOMAINS: ChatCoreV2Domain[] = ['training', 'cooking', 'content', 'finance'];

const SURFACE_VALUES = new Set<ChatCoreV2AllowedSurface>(DEFAULT_ALLOWED_SURFACES);
const DOMAIN_VALUES = new Set<ChatCoreV2Domain>(DEFAULT_ALLOWED_DOMAINS);
...
export function resolveChatCoreV2ActivationConfig(env: EnvLike = process.env): ChatCoreV2ActivationConfig {
  ...
  const defaultAllowedDomains = mode === 'on'
    ? DEFAULT_ALLOWED_DOMAINS
    : DEFAULT_SHADOW_CANARY_DOMAINS;
  const config: ChatCoreV2ActivationConfig = {
    ...
    allowedDomains: parseEnumList(env.CHAT_CORE_V2_ALLOWED_DOMAINS, DOMAIN_VALUES, defaultAllowedDomains),
    ...
    allowWriteExecution: parseBoolean(env.CHAT_CORE_V2_ALLOW_WRITE_EXECUTION, mode === 'on'),
    allowCloudFallback: parseBoolean(env.CHAT_CORE_V2_ALLOW_CLOUD_FALLBACK, false),
    legacyFallbackDisabled: parseBoolean(env.CHAT_CORE_V2_LEGACY_FALLBACK_DISABLED, false),
...
export interface ChatCoreV2TenantOverride {
  ...
  allowedDomains?: ChatCoreV2Domain[];
  /**
   * WP-20 per-tenant retirement flag. True disables the legacy `routeMessage`
   * fallback for THIS tenant only after earlier token-zero/v2 fast paths miss.
   * False can explicitly keep one tenant on legacy even if an env rollout list
   * includes other tenants. It is ignored when the mode/kill-switch is off.
   */
  legacyFallbackDisabled?: boolean;
}
...
/**
 * Resolve the requesting tenant's allowed domains as a narrow-only intersection
 * of the env allowlist and any per-tenant runtime override. Runtime overrides
 * can demote a tenant to fewer domains, never widen it past the global config.
 */
export function resolveChatCoreV2AllowedDomainsForTenant(
  env: EnvLike = process.env,
  tenantId?: string | number,
): Set<ChatCoreV2Domain> {
  const globalDomains = resolveChatCoreV2ActivationConfig(env).allowedDomains;
  const override = tenantId === undefined
    ? undefined
    : _runtimeOverrides.get(String(tenantId))?.allowedDomains;
  if (!override) return new Set<ChatCoreV2Domain>(globalDomains);

  const overrideSet = new Set<ChatCoreV2Domain>(override);
  return new Set<ChatCoreV2Domain>(globalDomains.filter((domain) => overrideSet.has(domain)));
}
...
/**
 * WP-20 (§5.J). Resolve the legacy-fallback retirement flag per tenant. The env
 * flag is default-off and only takes effect for tenants listed in
 * CHAT_CORE_V2_LEGACY_FALLBACK_DISABLED_TENANTS, so one tenant's retirement
 * rollout cannot starve another tenant. A runtime override can disable/enable
 * it for a single tenant without widening beyond canary/on or the kill-switch.
 */
export function isChatCoreV2LegacyFallbackDisabledForTenant(
  env: EnvLike = process.env,
  tenantId?: string | number,
): boolean {
  const tenantKey = tenantId === undefined ? undefined : String(tenantId);
  const config = resolveChatCoreV2ActivationConfig(env);
  if (config.mode !== 'canary' && config.mode !== 'on') return false;
  if (isChatCoreV2MasterKillSwitchOff(env, tenantKey)) return false;

  if (tenantKey !== undefined) {
    const override = _runtimeOverrides.get(tenantKey);
    if (override?.legacyFallbackDisabled !== undefined) {
      return override.legacyFallbackDisabled === true;
    }
  }

  if (!config.legacyFallbackDisabled || tenantKey === undefined) return false;
  return parseStringSet(env.CHAT_CORE_V2_LEGACY_FALLBACK_DISABLED_TENANTS).has(tenantKey);
}
...
function parseStringSet(raw: string | undefined): Set<string> {
  return new Set(
    (raw ?? '')
      .split(',')
      .map((value) => value.trim())
