...
export function loadShadowReplayCorpusItems(
  ...
  for (const row of rows) {
    ...
    // Candidate ids the selector produced at shadow time. Prefer the route
    // decision's `selectedCapabilityIds`; fall back to the guessed set so a row
    // is still scorable even if the response shape changed.
    const candidateCapabilityIds = normalizeCapabilityIds(
      response?.selectedCapabilityIds ?? contextPack?.guessedCapabilities,
    );
