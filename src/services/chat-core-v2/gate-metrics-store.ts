 ...
 * gate even if it were persisted — only a recall measured over a non-synthetic
 * (peer-reviewed real) corpus opens the gate.
 *
...
export interface ChatCoreV2GateReadinessReport {
   ...
   * AND a persisted recall exists and meets the target AND that recall is NOT
   * bound to the synthetic-seed content-hash. FALSE by construction today (no
   * persisted recall yet — WP-19-seed).
   */
...
export function measureChatCoreV2ShadowGateReadiness(
  ...
  const recallBoundToSyntheticSeed =
    ...
    && recallCorpusContentHash === getSyntheticSeedCorpusContentHash();

  // A recall opens the gate only if it EXISTS, meets the target, AND is NOT
  // bound to the synthetic seed. A synthetic-bound recall is treated as if it
  // does not meet the target for promotion purposes.
  ...
  const gateCanPromote = shadowReadinessMet && recallMeetsTarget;
...
function buildGateNotes(input: {
  ...
  if (input.gateCanPromote) {
    return `gateCanPromote=true: shadow readiness met AND persisted recall@8 ${input.persistedRecallAt8?.toFixed(4)} >= ${input.recallTarget} over a non-synthetic corpus`;
  }
