...
export function buildFinanceSummaryRoute(
  ...
  const readModel = buildChatCoreV2ReadModelResult<ChatCoreV2FinanceSummaryData>({
    ...
    domain: 'finance',
    data,
    sourceEntityIds: [financeSummaryEntityId(month)],
    sourceVersions: { [financeSummaryEntityId(month)]: hashStable(data) },
    generatedAt: now.toISOString(),
    maxSourceAgeSeconds: 60,
    sensitivity: 'financial',
    summary: buildFinanceSummaryText(data, input.locale),
  ...
  const response = buildChatCoreV2MessageResponse({
    text: readModel.summary ?? buildFinanceSummaryText(data, input.locale),
    locale: input.locale,
    reasonCodes: ['deterministic_read', FINANCE_SUMMARY_CAPABILITY, ...policy.reasons],
  });
