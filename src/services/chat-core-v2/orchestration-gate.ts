...
function resolveAllowedDomainsForTenant(
  ...
  const override = getChatCoreV2RuntimeOverride(String(tenantId))?.allowedDomains;
  if (!override) {
    return new Set<ChatCoreV2Domain>(globalDomains);
  }
  // Intersect: keep only global domains this tenant's override also permits.
  // (Narrow-only — a domain in the override but not in the global set is
  // dropped, so the override can never widen the tenant past the env allowlist.)
