...
import { getActiveLocalModel } from '../ollama-model-policy';
...
export const CHAT_CORE_V2_MODEL_RESIDENCY_POLICIES: ChatCoreV2ModelResidencyPolicy[] = [
  {
    ...
    get defaultModel() { return getActiveLocalModel({ fresh: true }).ollamaTag; },
    defaultKeepAlive: '-1',
    foregroundAllowed: true,
    notes: 'Legacy role id for the always-loaded signed-manifest model.',
  },
  {
    ...
    notes: 'No second local model. Approved cloud routing or visible failure handles escalation.',
  },
  {
    // Legacy role identifier retained for stored/test contract compatibility.
    // It resolves to the active signed model and never selects a second model.
    role: 'escalation_35b',
    modelEnvKey: 'OLLAMA_MODEL',
    get defaultModel() { return getActiveLocalModel({ fresh: true }).ollamaTag; },
    defaultKeepAlive: '0',
    foregroundAllowed: false,
    notes: 'Legacy alias for background_escalation; no alternate local model is selectable.',
  },
  {
  ...
  },
  {
    ...
    defaultKeepAlive: '5m',
    foregroundAllowed: false,
    notes: 'May share the resident signed-model instance when prompt shape and queueing allow.',
  },
];
 ...
 *   '-1' → -1   (always loaded; the resident planner hot path)
 *   '5m' → 300  (short-lived classifier shadow residency)
 *   '0'  → 0    (background/unknown roles unload immediately)
 * Unknown roles default to 0 so a stale caller cannot pin local memory. The
 * foreground signed-manifest model stays loaded; complex work uses the approved cloud
...
export function resolveChatCoreV2ModelResidencyConfig(
  ...
  const activeModel = getActiveLocalModel({ fresh: true }).ollamaTag;
  return CHAT_CORE_V2_MODEL_RESIDENCY_POLICIES.map((policy) => ({
    role: policy.role,
    model: (env[policy.modelEnvKey] ?? activeModel).trim() || activeModel,
...
export function validateChatCoreV2ModelResidencyConfig(
  ...
  const activeModel = getActiveLocalModel({ fresh: true }).ollamaTag;
  if (config.some((entry) => entry.model !== activeModel)) {
