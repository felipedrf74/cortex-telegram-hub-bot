 ...
 *     injected (shadow+/sandbox) — off-mode inert by construction;
   *   - legacy-fallback increments come from active ChatCoreV2 live terminal
   *     routes (chat-message-routes.ts) AFTER the call site has already proven
   *     the env mode is canary/on AND the per-tenant master kill-switch is not
   *     forcing this tenant off — so the increment can never run on the off-mode
   *     live route.
...
/** Idempotent DDL for the attributed fallback sidecar table (migration 179). */
export function ensureChatCoreV2LegacyFallbackAttributionCounterTable(db: Database.Database = getDb()): void {
  db.exec(`
...
export function ensureChatCoreV2AutoRevertCounterTables(db: Database.Database = getDb()): void {
  ...
  ensureChatCoreV2LegacyFallbackAttributionCounterTable(db);
}
 ...
 * production callers have already proven the mode is canary/on and the
 * per-tenant kill-switch is not off before calling.
 *
 * Returns `true` when a row was written/updated, `false` when swallowed.
 */
...
export interface LegacyFallbackAttributionInput {
  fellBack: boolean;
  domain?: string | null;
...
/**
 * Increment the attributed fallback sidecar. This is diagnostic only; it does
 * not feed the broad auto-revert gauge. Labels are aggressively normalized to
 * enum-ish safe tokens so callers cannot persist raw prompt/content by mistake.
 */
export function incrementLegacyFallbackAttribution(
  db: Database.Database,
  tenantId: string,
  outcome: LegacyFallbackAttributionInput,
  now: Date = new Date(),
): boolean {
  try {
    ensureChatCoreV2LegacyFallbackAttributionCounterTable(db);
    ...
    db.prepare(
      `INSERT INTO chat_v2_legacy_fallback_attribution_counter
       ...
       VALUES (@tenantId, @windowStart, @domain, @routeOwner, @routeMethod, @fallbackDelta, 1, @updatedAt)
       ON CONFLICT(tenant_id, window_start, domain, route_owner, route_method) DO UPDATE SET
         fallback_count = fallback_count + @fallbackDelta,
         total_count = total_count + 1,
         updated_at = @updatedAt`,
    ).run({
      tenantId: String(tenantId),
      windowStart,
      domain: safeCounterLabel(outcome.domain),
      routeOwner: safeCounterLabel(outcome.routeOwner),
      routeMethod: safeCounterLabel(outcome.routeMethod),
      fallbackDelta,
      updatedAt: now.toISOString(),
    });
    return true;
    ...
    logger.warn(
      ...
      'Chat Core v2 attributed legacy-fallback counter increment failed (swallowed; turn unaffected)',
    );
    return false;
  }
}
...
export interface LegacyFallbackAttributionSums extends LegacyFallbackSums {
  domain: string;
  routeOwner: string;
  routeMethod: string;
}
...
/**
 * Return attributed fallback rows for one tenant/window. Diagnostic only; empty
 * or missing table returns [] so dashboards fail closed without blocking turns.
 */
export function listLegacyFallbackAttributionSince(
  db: Database.Database,
  tenantId: string,
  cutoffWindowStart: string,
): LegacyFallbackAttributionSums[] {
  try {
    const rows = db
      .prepare(
        `SELECT
           domain,
           route_owner AS routeOwner,
           route_method AS routeMethod,
           COALESCE(SUM(fallback_count), 0) AS fallback,
           COALESCE(SUM(total_count), 0)    AS total
         FROM chat_v2_legacy_fallback_attribution_counter
         WHERE tenant_id = ? AND window_start >= ?
         GROUP BY domain, route_owner, route_method
         ORDER BY fallback DESC, total DESC, domain ASC, route_owner ASC, route_method ASC`,
      )
      .all(String(tenantId), cutoffWindowStart) as LegacyFallbackAttributionSums[];
    return rows.map((row) => ({
      domain: safeCounterLabel(row.domain),
      routeOwner: safeCounterLabel(row.routeOwner),
      routeMethod: safeCounterLabel(row.routeMethod),
...
function safeCounterLabel(raw: unknown): string {
