 ...
 * can NEVER satisfy promotion. Only a recall measured over a non-synthetic,
 * peer-reviewed REAL corpus opens the gate — a data/process step out of code
 * scope. `runCorpusEval` surfaces `corpusIsSyntheticSeed` and the runner never
 * pretends otherwise.
 *
...
export interface CorpusEvalRunResult {
  ...
  /**
   * HONEST gate verdict: TRUE only when recall meets the target AND the corpus
   * is NOT the synthetic seed. A synthetic-bound recall is ALWAYS GATE_FAIL,
   * regardless of its numeric value — the synthetic seed can never open the gate.
   */
  gatePass: boolean;
...
export function runCorpusEval(
  ...
  for (let i = 0; i < goldenItems.length; i += 1) {
    ...
    merged.push({ message: key, expectedCapabilityIds: goldenItems[i].expectedCapabilityIds });
  }
  for (let i = 0; i < shadowItems.length; i += 1) {
    const key = `s:${i}`;
    candidateByKey.set(key, shadowItems[i].candidateCapabilityIds);
    merged.push({ message: key, expectedCapabilityIds: shadowItems[i].expectedCapabilityIds });
  }
  ...
  // and shadow items by {token, expectedCapabilityIds} — NEVER raw shadow text
  // (there is none). When the run is golden-seed-only, the hash equals
  // getSyntheticSeedCorpusContentHash() exactly. ─────────────────────────────
  const corpusContentHash = computeMergedCorpusContentHash(goldenCorpus, shadowItems);
  const corpusIsSyntheticSeed = corpusContentHash === getSyntheticSeedCorpusContentHash();

  // HONEST gate verdict: synthetic-bound recall is ALWAYS a fail.
  const recallMeetsTarget = recall.recallAtK >= gateTarget;
  const gatePass = recallMeetsTarget && !corpusIsSyntheticSeed;

  const runId = options.runId ?? deriveRunId(evalType, corpusContentHash, recordedAt);

  let wrotePersistedRecall = false;
  if (persist) {
    upsertRecallAt8(recall.recallAtK, { corpusContentHash, recordedAt }, db);
    recordEvalRun(db, {
      runId,
      evalType,
  ...
  return {
    ...
    gatePass,
    notes: buildVerdictNotes({ recallAtK: recall.recallAtK, gateTarget, corpusIsSyntheticSeed, gatePass }),
    recall,
    recordedAt,
  };
}
 ...
 * {salted token, expectedCapabilityIds} — NEVER raw shadow text. When there are
 * no shadow items, this equals `computeChatCoreV2CorpusContentHash(goldenCorpus)`
 * exactly.
 */
function computeMergedCorpusContentHash(
  ...
  const shadowCanonical = shadowItems
    .map((item) => `${item.messageToken}|${[...item.expectedCapabilityIds].sort().join(',')}`)
    .sort()
    .join(';');
...
function buildVerdictNotes(input: {
  ...
  if (input.gatePass) {
    return `GATE_PASS: recall@8 ${input.recallAtK.toFixed(4)} >= ${input.gateTarget} over a NON-synthetic corpus.`;
  }
