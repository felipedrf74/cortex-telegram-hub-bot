...
const PHOTO_EXTRACTION_BASE64_PATTERN = /^[A-Za-z0-9+/]+={0,2}$/;

function resolvePhotoExtractionMaxBase64Chars(): number {
  const configured = Number(process.env.PHOTO_EXTRACTION_MAX_BASE64_CHARS);
  if (Number.isFinite(configured) && configured > 0) {
    return Math.min(Math.floor(configured), PHOTO_EXTRACTION_MAX_BASE64_CHARS);
  }
  return PHOTO_EXTRACTION_MAX_BASE64_CHARS;
}
...
export function normalizePhotoExtractionAttachment(raw: unknown): PhotoExtractionAttachment | null {
  ...
  if (base64.length > resolvePhotoExtractionMaxBase64Chars()) return null;
  if (!PHOTO_EXTRACTION_BASE64_PATTERN.test(base64)) return null;
