...
} from './training-plan-equipment-adaptation';
...
} from './training-plan-coordination';
import {
...
import {
  ...
  type BusyWindow,
} from '../api/routes/training-schedule-utils';
import {
...
} from '../api/routes/training-profile-requirements';
import { buildDeterministicTrainingPlan } from '../api/routes/training-fallback-plan';
import { fetchCurrentReadinessForPlan } from '../api/routes/training-read-models';
import {
...
} from '../api/routes/training-plan-persistence';
import {
...
} from './training-plan-volume-enforcement';
import { loadTrainingCatalogSnapshot } from './coach-kernel/training-catalog';
import * as trainingPlans from './training-plans';
import { resolveTrainingCalendarSource } from './training-calendar-source';
import type { TrainingPlanGenerationLeaseIdentity } from './training-plan-generation-idempotency';
import {
  assessTrainingPlanSpecReadiness,
  buildTrainingPlanSpec,
  type EnduranceKeyDay,
  type TrainingPlanSpecReadinessResult,
} from './training-plan-spec';
import { parseSessionDurationMinutesAnswer } from './training-plan-clarification-registry';
import {
  mergeTrainingQualityIntoPlanLint,
  prepareTrainingPlanForQualityGate,
} from './coach-kernel/training-plan-quality-gate';
import {
  isFutureIsoDate,
  isStrictIsoDate,
...
import { getEffectiveHealthSafetyOutput } from './health-data-lifecycle';
import { incrementTrainingGenerationCounter } from './training-generation-observability';
import type { WireHealthSignalOutput } from './coach-kernel/safety-wiring';
import {
  attachTrainingLearningPathToPlan,
  type TrainingLearningPath,
} from './training-learning-path';
import {
...
interface TrainingPlanGenerationSnapshot {
  readonly expectedActivePlanIds: ReadonlyArray<number>;
  readonly profiles: Readonly<{
    fitness: Record<string, any> | null;
    gym: Record<string, any> | null;
    running: Record<string, any> | null;
    cycling: Record<string, any> | null;
    swimming: Record<string, any> | null;
  }>;
  readonly profileRequirements: Readonly<Record<string, Readonly<{
    title: string;
    missingFields: ReadonlyArray<unknown>;
  }>>>;
  readonly readiness: Awaited<ReturnType<typeof fetchCurrentReadinessForPlan>>;
  readonly catalog: ReturnType<typeof loadTrainingCatalogSnapshot>;
  readonly plannerNow: string;
  ...
  readonly history: Readonly<{
    real: RealTrainingHistory | null;
    compliance: RecentTrainingComplianceRead | null;
  }>;
  readonly coordination: Readonly<{
    training: Awaited<ReturnType<typeof readTrainingMeshContext>> | null;
    cooking: Awaited<ReturnType<typeof readCookingMeshContext>> | null;
    finance: Awaited<ReturnType<typeof readFinanceMeshContext>> | null;
    content: Awaited<ReturnType<typeof readContentMeshContext>> | null;
    secretary: Awaited<ReturnType<typeof readSecretaryMeshContext>> | null;
    sharedDecisionContext: string;
  }>;
  readonly safetyOutput: WireHealthSignalOutput | null;
}
...
type TrainingPlanGeneratedCandidate =
  | Readonly<{
      pipelineStage: 'candidate';
      disposition: 'preview';
      signPreview(): TrainingPlanGenerationResult;
    }>
  | Readonly<{
      pipelineStage: 'candidate';
      disposition: 'persist';
      persist(): Promise<TrainingPlanGenerationResult>;
    }>;

const trainingPlanGenerationOrchestrator = new TrainingPlanGenerationOrchestrator<
  GenerateTrainingPlanForUserInput,
  TrainingPlanGenerationResult,
  TrainingPlanGeneratedCandidate,
  TrainingPlanGenerationSnapshot
>({
  generateCandidate: async (context) => {
    const generated = await buildTrainingPlanGenerationCandidate(context);
    return isGeneratedCandidate(generated)
      ? {
          kind: 'candidate',
          candidate: generated,
          disposition: generated.disposition,
        }
      : { kind: 'complete', result: generated };
  },
  signPreview: async (_context, candidate) => {
    if (candidate.disposition !== 'preview') {
      throw new Error('Training plan generation candidate is not preview-signable');
    }
    return candidate.signPreview();
  },
  persist: async (_context, candidate) => {
    if (candidate.disposition !== 'persist') {
      throw new Error('Training plan generation candidate is not persistable');
    }
  ...
  captureSnapshot: async (request) => {
    const tenantId = requireTenantIdParam(
      request.tenantId,
      'TrainingPlanGenerationOrchestrator.captureSnapshot',
    );
    const expectedActivePlanIds = request.previewOnly
      ? []
      : trainingPlans.getActivePlans(request.userId, tenantId)
        .map((plan) => plan.id)
        .sort((left, right) => left - right);
    const profiles = {
      fitness: unwrapOnboardingProfileData(onboarding.getProfile?.(request.userId, 'fitness')),
      gym: unwrapOnboardingProfileData(onboarding.getProfile?.(request.userId, 'triathlon-gym')),
      running: unwrapOnboardingProfileData(onboarding.getProfile?.(request.userId, 'triathlon-running')),
      cycling: unwrapOnboardingProfileData(onboarding.getProfile?.(request.userId, 'triathlon-cycling')),
      swimming: unwrapOnboardingProfileData(onboarding.getProfile?.(request.userId, 'triathlon-swim')),
    };
    const profileRequirements = Object.fromEntries(
      ['fitness', 'triathlon-gym', 'triathlon-running', 'triathlon-cycling', 'triathlon-swim']
        .map((questionnaireId) => [questionnaireId, {
          title: onboarding.getQuestionnaire(questionnaireId)?.title ?? questionnaireId,
          missingFields: onboarding.getMissingProfileFields?.(request.userId, questionnaireId) ?? [],
        }]),
    );
    const plannerNow = resolvePlannerNow(request.plannerNow);
    const [readiness, calendar] = await Promise.all([
      fetchCurrentReadinessForPlan(request.userId, tenantId),
      captureTrainingPlanCalendarSnapshot({
        request,
        tenantId,
        profiles,
        plannerNow,
    ...
    const coordination = await captureTrainingPlanCoordinationSnapshot({
      userId: request.userId,
      tenantId,
      weekStart: calendar?.startDate ?? resolveTrainingPlanStartDate(
        plannerNow,
        normalizeStartPolicy(request.startPolicy),
        resolveTrainingTimezone(request.schedulingTimezone),
      ),
    });
    let realHistory: RealTrainingHistory | null = null;
    let recentCompliance: RecentTrainingComplianceRead | null = null;
    try {
      realHistory = readTrainingHistoryFromCompletions(request.userId, { tenantId });
      recentCompliance = readTrainingComplianceFromRecentHistory(request.userId, { tenantId });
    } catch (error) {
      logger.warn({
        errorName: error instanceof Error ? error.name : 'unknown',
        userId: request.userId,
        tenantId,
      }, 'Training plan generation history snapshot unavailable');
    }
    return {
      ...
      profileRequirements,
      readiness,
      catalog: loadTrainingCatalogSnapshot({ tenantId }),
      plannerNow: plannerNow.toISOString(),
      calendar,
      history: {
        real: realHistory,
      ...
      coordination,
      safetyOutput: config.coaching.trainingSafetyGuardrailsEnabled && calendar
        ? getEffectiveHealthSafetyOutput({
            userId: request.userId,
            tenantId,
            affectedDate: calendar.startDate,
...
/**
 * Compatibility export retained for released callers. All work now enters
 * through the tenant-scoped orchestrator, which captures one immutable
 * request/evidence context before the existing validation, deterministic
 * kernel, preview signing, and persistence sequence runs.
 */
export async function generateTrainingPlanForUser(
  ...
  return trainingPlanGenerationOrchestrator.execute(input);
}

function isGeneratedCandidate(
  value: TrainingPlanGenerationResult | TrainingPlanGeneratedCandidate,
): value is TrainingPlanGeneratedCandidate {
  return 'pipelineStage' in value && value.pipelineStage === 'candidate';
}

async function captureTrainingPlanCalendarSnapshot(input: {
  request: Readonly<GenerateTrainingPlanForUserInput>;
  tenantId: number;
  profiles: TrainingPlanGenerationSnapshot['profiles'];
  plannerNow: Date;
}): Promise<TrainingPlanGenerationSnapshot['calendar']> {
  if (!input.profiles.fitness || Object.keys(input.profiles.fitness).length === 0) return null;

  const schedulingTimezone = resolveTrainingTimezone(input.request.schedulingTimezone);
  const normalizedRaceDate = normalizeIsoDate(input.request.raceDate);
  const raceDateWasProvided = typeof input.request.raceDate === 'string'
    ? input.request.raceDate.trim() !== ''
    : input.request.raceDate != null;
  if (raceDateWasProvided && normalizedRaceDate == null) return null;
  if (normalizedRaceDate
    && !isFutureIsoDate(normalizedRaceDate, input.plannerNow, schedulingTimezone)) return null;

  const profileRaceDate = resolveProfileRaceDate(input.profiles.running);
  const effectiveProfileRaceDate = profileRaceDate
    && isFutureIsoDate(profileRaceDate, input.plannerNow, schedulingTimezone)
    ? profileRaceDate
    : null;
  const effectiveRaceDate = normalizedRaceDate ?? effectiveProfileRaceDate;
  const startDate = resolveTrainingPlanStartDate(
    input.plannerNow,
    normalizeStartPolicy(input.request.startPolicy),
    schedulingTimezone,
  );
  if (effectiveRaceDate && effectiveRaceDate < startDate) return null;

  const requestedDurationWeeks = normalizeTrainingPlanDurationWeeks(input.request.durationWeeks, 4);
  const durationWeeks = clampTrainingPlanDurationWeeksToRaceDate({
    requestedDurationWeeks,
    startDateIso: startDate,
    raceDateIso: effectiveRaceDate,
    schedulingTimezone,
  });
  const endDate = DateTime
    .fromISO(startDate, { zone: schedulingTimezone })
    .plus({ weeks: durationWeeks })
    .toISODate() ?? startDate;
  const source = resolveTrainingCalendarSource({
    userId: input.request.userId,
    tenantId: input.tenantId,
    requestedSource: input.request.calendarSource ?? undefined,
  });

  try {
    const events = source
      ? await getEventsForSources(startDate, endDate, input.request.userId, [source])
      : await getEvents(startDate, endDate, input.request.userId);
    return {
      startDate,
      endDate,
      source: source ?? null,
    ...
    logger.warn({
      ...
      endDate,
      errorName: error instanceof Error ? error.name : 'unknown',
    }, 'Training plan generation calendar snapshot unavailable');
    return {
      startDate,
      endDate,
      source: source ?? null,
...
async function captureTrainingPlanCoordinationSnapshot(input: {
  userId: number;
  tenantId: number;
  weekStart: string;
}): Promise<TrainingPlanGenerationSnapshot['coordination']> {
  const results = await Promise.allSettled([
    readTrainingMeshContext(input),
    readCookingMeshContext(input),
    readFinanceMeshContext(input),
    readContentMeshContext(input),
    readSecretaryMeshContext(input),
    buildSharedDecisionContext('triathlon', input.userId, input.tenantId),
  ] as const);
  const rejected = results
    .map((result, index) => result.status === 'rejected'
      ? { index, errorName: result.reason instanceof Error ? result.reason.name : 'unknown' }
      : null)
    .filter((value): value is { index: number; errorName: string } => value !== null);
  if (rejected.length > 0) {
    logger.warn({
      userId: input.userId,
      tenantId: input.tenantId,
      failedInputs: rejected,
    }, 'Training plan generation coordination snapshot partially unavailable');
  }
  return {
    training: results[0].status === 'fulfilled' ? results[0].value : null,
    cooking: results[1].status === 'fulfilled' ? results[1].value : null,
    finance: results[2].status === 'fulfilled' ? results[2].value : null,
    content: results[3].status === 'fulfilled' ? results[3].value : null,
    secretary: results[4].status === 'fulfilled' ? results[4].value : null,
    sharedDecisionContext: results[5].status === 'fulfilled' ? results[5].value : '',
  };
}

async function buildTrainingPlanGenerationCandidate(
  context: TrainingPlanGenerationContext<
    GenerateTrainingPlanForUserInput,
    TrainingPlanGenerationSnapshot
  >,
): Promise<TrainingPlanGenerationResult | TrainingPlanGeneratedCandidate> {
  const input = context.request as GenerateTrainingPlanForUserInput;
  const {
    userId,
    objective,
    preferredTime = '12:00',
  ...
  const expectedActivePlanIds = [...context.snapshot.expectedActivePlanIds];
  ...
  const now = new Date(context.snapshot.plannerNow);

  const fitnessProfile = context.snapshot.profiles.fitness;
  const gymProfile = context.snapshot.profiles.gym;
  const runProfile = context.snapshot.profiles.running;
  const cyclingProfile = context.snapshot.profiles.cycling;
  const swimProfile = context.snapshot.profiles.swimming;
  const capturedProfilesByQuestionnaire: Record<string, Record<string, any> | null> = {
    fitness: fitnessProfile,
    'triathlon-gym': gymProfile,
    'triathlon-running': runProfile,
    'triathlon-cycling': cyclingProfile,
    'triathlon-swim': swimProfile,
  };
  ...
  if (!fitnessProfile || Object.keys(fitnessProfile).length === 0) {
    ...
    return {
      ...
      data: {
        ...
        requiredQuestionnaireTitle: context.snapshot.profileRequirements.fitness?.title ?? 'fitness',
        message: 'Complete your Fitness Profile first to generate a personalized plan.',
        missingFields: context.snapshot.profileRequirements.fitness?.missingFields ?? [],
      },
    };
  }

  const objectiveRequirement = resolveObjectiveProfileRequirement(objective, userId, {
    getProfile: (_capturedUserId, questionnaireId) => capturedProfilesByQuestionnaire[questionnaireId] ?? null,
    getMissingProfileFields: (_capturedUserId, questionnaireId) =>
      [...(context.snapshot.profileRequirements[questionnaireId]?.missingFields ?? [])],
    getQuestionnaire: (questionnaireId) => ({
      title: context.snapshot.profileRequirements[questionnaireId]?.title ?? questionnaireId,
    }),
  });
  ...
  const endStr = DateTime
    ...
    .plus({ weeks: durationWeeks })
    .toISODate() ?? startStr;
  const calendarSnapshot = context.snapshot.calendar;
  if (!calendarSnapshot
    || calendarSnapshot.startDate !== startStr
    || calendarSnapshot.endDate !== endStr) {
    throw new Error('Training plan generation calendar snapshot does not match validated plan dates');
  }
  const resolvedCalendarSource = calendarSnapshot.source;
  const busyWindows: BusyWindow[] = [...calendarSnapshot.busyWindows];
  const calendarFetchDegraded = calendarSnapshot.degraded;
  const calendarFetchError = calendarSnapshot.error;
  ...
  const capturedCoordination = context.snapshot.coordination;
  const sharedDecisionContext = capturedCoordination.sharedDecisionContext;
  const coordination = buildTrainingPlanCoordination({
    ...
    training: capturedCoordination.training,
    cooking: capturedCoordination.cooking,
    finance: capturedCoordination.finance,
    content: capturedCoordination.content,
    secretary: capturedCoordination.secretary,
  ...
  const currentReadiness = context.snapshot.readiness;
  ...
  try {
    kernelEquipmentCandidate = applyTrainingPlanCoordination(buildCoachKernelTrainingPlan({
      ...
      secretaryAgendaItems: context.secretaryAgendaItems,
      realTrainingHistory: context.snapshot.history.real,
      recentTrainingCompliance: context.snapshot.history.compliance,
  ...
  const safetyOutput = config.coaching.trainingSafetyGuardrailsEnabled
    ? context.snapshot.safetyOutput ?? undefined
    : undefined;
  ...
  if (previewOnly) {
    return Object.freeze({
      pipelineStage: 'candidate' as const,
  ...
  return Object.freeze({
    pipelineStage: 'candidate' as const,
    disposition: 'persist' as const,
    persist: async (): Promise<TrainingPlanGenerationResult> => {
...
function buildTrainingGenerationVersionPins(
  snapshot: ReturnType<typeof loadTrainingCatalogSnapshot>,
