-- Migration 248: extend the closed aggregate-only Content observability
-- taxonomy for the temporary rollout gate and legacy-route exit counters.
-- No identity, timestamp, content, prompt, URL, hash, or provider response is
-- stored. Existing aggregate totals are preserved exactly.

CREATE TABLE content_workspace_operation_metrics_v3 (
  operation TEXT PRIMARY KEY CHECK (operation IN (
    ...
    'schedule_preview', 'schedule_confirm', 'schedule_cancel', 'rollout_gate'
  )),
...
INSERT INTO content_workspace_operation_metrics_v3
SELECT * FROM content_workspace_operation_metrics;
DROP TABLE content_workspace_operation_metrics;
ALTER TABLE content_workspace_operation_metrics_v3 RENAME TO content_workspace_operation_metrics;

CREATE TABLE content_workspace_reason_metrics_v3 (
...
INSERT INTO content_workspace_reason_metrics_v3
SELECT * FROM content_workspace_reason_metrics;
DROP TABLE content_workspace_reason_metrics;
ALTER TABLE content_workspace_reason_metrics_v3 RENAME TO content_workspace_reason_metrics;
