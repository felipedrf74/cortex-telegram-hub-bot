#!/usr/bin/env bash
# Run every SQLite-bearing Training evidence writer inside the backend's lock domain.
...
run_in_backend node scripts/training-e2e-flow.mjs
run_in_backend npx tsx scripts/training-e2e-quality.ts
node scripts/training-e2e-verify-freshness.mjs
