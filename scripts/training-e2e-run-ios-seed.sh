#!/usr/bin/env bash
# Run an iOS fixture lifecycle operation inside the backend SQLite lock domain.

set -euo pipefail

MODE="${1:-}"
case "$MODE" in
  prepare|cleanup|prepare-clarification|verify-clarification|cleanup-clarification)
    ;;
  *)
    echo "ERROR: unsupported Training iOS seed mode '$MODE'." >&2
    exit 64
    ;;
esac
...
if [[ -n "${NEXUS_TRAINING_E2E_IOS_SEED_ATTEMPT_ID:-}" ]]; then
  EXEC_ENV+=(-e NEXUS_TRAINING_E2E_IOS_SEED_ATTEMPT_ID="$NEXUS_TRAINING_E2E_IOS_SEED_ATTEMPT_ID")
fi

# Host-side source/image attestation brackets the container-owned write. A
# failed seed remains the primary status even if the post-check also fails.
...
docker compose --project-name "$NEXUS_TRAINING_E2E_PROJECT" "${COMPOSE_FILES[@]}" exec -T \
  "${EXEC_ENV[@]}" \
  nexus-hub node scripts/training-e2e-ios-seed.mjs "$MODE"
seed_status=$?
node scripts/training-e2e-verify-freshness.mjs
freshness_status=$?
set -e
