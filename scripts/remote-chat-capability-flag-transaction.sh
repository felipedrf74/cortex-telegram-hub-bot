...
SYNTHETIC_QA_MANIFEST_FILE=''
SYNTHETIC_QA_RECEIPT_FILE=''
...
case "$COMMAND" in
  inspect)
    { [ "$#" -eq 3 ] || [ "$#" -eq 6 ]; } \
      || die 'inspect requires flag, value, transition reason, and only an optional routing window plus synthetic QA manifest digest'
    FLAG="$1"
    DESIRED_VALUE="$2"
    TRANSITION_REASON="$3"
    if [ "$#" -eq 6 ]; then
      SINCE="$4"
      UNTIL="$5"
      SYNTHETIC_QA_MANIFEST_SHA256="$6"
    fi
    ...
    case "$FLAG" in
      AI_ROUTING_MANIFEST_CLASSIFIER|AI_ROUTING_MANIFEST_ORCHESTRATOR|AI_ROUTING_MANIFEST_SHADOW|AI_ROUTING_MANIFEST_REGISTRY)
        if [ "$ROLE" = staging ] && [ "$DESIRED_VALUE" = true ]; then
          ...
          [[ "$SYNTHETIC_QA_MANIFEST_SHA256" =~ ^sha256:[0-9a-f]{64}$ ]] \
            || die 'staging routing enable inspect requires one exact synthetic QA manifest digest'
      ...
      *)
        [ -z "$SINCE" ] && [ -z "$UNTIL" ] \
          && [ -z "$SYNTHETIC_QA_MANIFEST_SHA256" ] \
...
NODE
  ...
  (
    ...
    env -i \
      ...
      "$NODE_BIN" --env-file="$STAGING_ENV_FILE" scripts/routing-divergence-report.mjs \
        ...
        --synthetic-qa-manifest="$SYNTHETIC_QA_MANIFEST_FILE" \
        --expected-synthetic-qa-manifest-sha256="$SYNTHETIC_QA_MANIFEST_SHA256" \
        --synthetic-qa-receipt="$SYNTHETIC_QA_RECEIPT_FILE" \
        --gate --json > "$output"
  )
}
...
collect_native_evidence_sources() {
  ...
  case "$ROLE:$FLAG" in
    staging:AI_ROUTING_MANIFEST_CLASSIFIER|staging:AI_ROUTING_MANIFEST_ORCHESTRATOR|staging:AI_ROUTING_MANIFEST_SHADOW|staging:AI_ROUTING_MANIFEST_REGISTRY)
      local qa_surface qa_digest qa_root
      qa_surface="$(routing_surface)" || die 'routing synthetic QA surface is unavailable'
      qa_digest="${SYNTHETIC_QA_MANIFEST_SHA256#sha256:}"
      [[ "$qa_digest" =~ ^[0-9a-f]{64}$ ]] \
        || die 'routing synthetic QA manifest digest is invalid'
      qa_root="$ROUTING_SYNTHETIC_QA_STATE_ROOT/$RUNTIME_SHA-${ARTIFACT_DIGEST:0:12}/$qa_surface"
      [ -d "$qa_root" ] && [ ! -L "$qa_root" ] \
        && [ "$(readlink -f "$qa_root")" = "$qa_root" ] \
        || die 'routing synthetic QA evidence directory is unsafe'
      SYNTHETIC_QA_MANIFEST_FILE="$qa_root/$qa_digest.manifest.json"
      SYNTHETIC_QA_RECEIPT_FILE="$qa_root/$qa_digest.receipt.json"
      for qa_file in "$SYNTHETIC_QA_MANIFEST_FILE" "$SYNTHETIC_QA_RECEIPT_FILE"; do
        [ -f "$qa_file" ] && [ ! -L "$qa_file" ] \
          || die 'exact routing synthetic QA evidence is unavailable'
        [ "$(stat -c '%U:%a:%h' "$qa_file")" = "$(id -un):600:1" ] \
          || die 'routing synthetic QA evidence owner, mode, or link count is unsafe'
      done
      "$NODE_BIN" - "$SYNTHETIC_QA_MANIFEST_FILE" "$qa_digest" <<'NODE' \
        || die 'routing synthetic QA manifest bytes do not match the precommitted SHA-256 digest'
const crypto = require('node:crypto');
const fs = require('node:fs');
const [manifestFile, expectedDigest] = process.argv.slice(2);
const actualDigest = crypto.createHash('sha256').update(fs.readFileSync(manifestFile)).digest('hex');
if (actualDigest !== expectedDigest) process.exit(1);
NODE
