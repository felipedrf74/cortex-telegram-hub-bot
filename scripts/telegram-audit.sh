...
echo "  [x] Stage C: migrations/258_telegram_identity_archive.sql archives"
...
echo "      bootstrap-preflight.ts, src/services/database.ts comment."
...
echo "  [ ] LIVE TELEGRAM_* disable-sentinel contract:"
echo "      src/services/content-live-evaluation-runtime.ts refuses to boot the"
echo "      content live-eval runtime unless TELEGRAM_LEGACY_DELIVERY='false'"
echo "      AND TELEGRAM_BOT_TOKEN='content-live-eval-disabled'. Those"
echo "      sentinels are exported by scripts/content-live-eval-local.sh,"
echo "      scripts/full-nexus-local-engine.sh, scripts/decision-center-ios-"
echo "      smoke.sh, and scripts/chat-tenant-security-smoke.js (plus disabled"
echo "      placeholder tokens in seed-training-catalog.ts, training-e2e-live-"
echo "      calendar.ts, and the release-*-container.sh test env). This is a"
echo "      live safety contract, not dead vocabulary — it goes away only when"
echo "      the sentinel pair is renamed to a non-telegram key in the runtime"
echo "      assert AND every script above, in the same change."
echo "  Gate: __tests__/scripts/telegram-purge-gate.test.ts pins Stage A+B"
echo "  files at zero live refs and pins migration 258 as archive-first."
