...
for relative in dist prompts migrations package.json package-lock.json ecosystem.config.js; do
  ...
  rsync -a --chmod=D700,Fu+rw,go-rwx "$RUNTIME_DIR/$relative" "$STAGE/$(dirname "$relative")/"
done
rsync -a --chmod=D700,Fu+rw,go-rwx \
...
if [ -d "$RUNTIME_DIR/catalog" ]; then
  rsync -a --chmod=D700,Fu+rw,go-rwx "$RUNTIME_DIR/catalog/" "$STAGE/catalog/"
fi
