 ...
 *     `upsertRecallAt8()`, first called by the WP-19 seed/weekly eval path. This
 *     test must never import or call that persistence writer.
 *
...
describe('Chat Core v2 prepass recall@8 CI assertion (WP-09 / B4)', () => {
  ...
  it('does NOT persist recall (WP-19 owns the writer)', () => {
    ...
    // only asserts. Persistence belongs in WP-19 seed/weekly eval paths, not
    // here — so this module must contain no `import ... from` statement that
    // pulls in a gate-metrics writer/store. (We check imports, not any mention,
    // so this self-documenting comment cannot trip the guard.)
