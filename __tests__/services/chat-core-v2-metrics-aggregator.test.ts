...
describe('WP-06 scheduler wiring (default-off, compute+log only, no raw PII)', () => {
  ...
  it('registers chat_v2_auto_revert_eval at */5 only when mode !== off', () => {
    expect(source).toContain(
      "registerJob('chat_v2_auto_revert_eval', 'Chat Core v2 Auto-Revert Eval', '*/5 * * * *', 'system')",
    );
    // The registerJob call sits inside a mode-gate guard.
    expect(source).toMatch(
      /resolveChatCoreV2ActivationConfig\(process\.env\)\.mode !== 'off'\)\s*\{\s*registerJob\('chat_v2_auto_revert_eval'/,
    );
    // The cron.schedule body is likewise gated by the same mode check.
    const cronGate = source.match(
      /if \(resolveChatCoreV2ActivationConfig\(process\.env\)\.mode !== 'off'\) \{\s*cron\.schedule\('\*\/5 \* \* \* \*', wrapJob\('chat_v2_auto_revert_eval'/,
    );
