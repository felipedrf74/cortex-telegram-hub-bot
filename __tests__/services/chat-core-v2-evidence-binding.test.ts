...
import {
  ...
  CHAT_CORE_V2_DOMAIN_EVIDENCE_TAXONOMY,
...
describe('Chat Core v2 evidence binding (WP-05)', () => {
  ...
  describe('ChatCoreV2DomainEvidenceTaxonomy', () => {
    it('covers secretary/training/content/cooking/tasks/finance', () => {
    ...
    it('keeps every production-chat domain grounded by read models and policies', () => {
      for (const kinds of Object.values(CHAT_CORE_V2_DOMAIN_EVIDENCE_TAXONOMY)) {
        expect(kinds).toContain('read_model');
        expect(kinds).toContain('system_policy');
      }
      expect(CHAT_CORE_V2_DOMAIN_EVIDENCE_TAXONOMY.finance).not.toContain('memory');
      expect(CHAT_CORE_V2_DOMAIN_EVIDENCE_TAXONOMY.content).toEqual(expect.arrayContaining([
        'decision_text',
        'user_attachment',
      ]));
    });
