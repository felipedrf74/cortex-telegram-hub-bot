...
describe('ChatActionPlanner multi-step DAG', () => {
  ...
  it('turns two safe-write segments into a stable sequential DAG without confirmation', async () => {
  ...
  it('emits multi-step result metadata and continues independent siblings after a blocked dependency', async () => {
  ...
  it('executes clear safe-write siblings while surfacing one follow-up for unresolved safe-write steps', async () => {
    const sourcePlan: ChatActionPlan = {
      schemaVersion: 1,
      userId: '42',
      tenantId: '42',
    ...
    expect(response.text).toContain('I completed the parts that were clear');
    ...
    expect(response.metadata.multiStepSummary).toMatchObject({
      ...
      perStep: [
        { stepId: 'step_1', status: 'verified_success' },
        { stepId: 'step_2', status: 'needs_clarification' },
        { stepId: 'step_3', status: 'verified_success' },
      ],
    });
  });
