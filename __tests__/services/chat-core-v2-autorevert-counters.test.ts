...
import {
  ...
  ensureChatCoreV2LegacyFallbackAttributionCounterTable,
  ...
  listLegacyFallbackAttributionSince,
...
function fallbackAttributionRowCount(db: Database.Database): number {
  return (db.prepare('SELECT COUNT(*) AS n FROM chat_v2_legacy_fallback_attribution_counter').get() as { n: number }).n;
}
...
describe('autorevert-counters — REVERT-SAFE empty-table defaults (load-bearing)', () => {
  ...
  it('EMPTY tables → computeSchemaComplianceRate1h === 1.0 AND computeLegacyFallbackRate24h === 0.0', () => {
    ...
    expect(fallbackAttributionRowCount(db)).toBe(0);
...
describe('autorevert-counters — attributed legacy-fallback sidecar', () => {
  let db: Database.Database;
  beforeEach(() => {
    db = makeDb();
  });
  ...
  it('increments safe domain/route attribution in the same hour bucket', () => {
    incrementLegacyFallbackAttribution(
      db,
      'tenant-a',
      {
        fellBack: true,
        domain: 'Tasks / comprar suplementos',
        routeOwner: 'Legacy Action Planner',
        routeMethod: 'needs clarification!',
      },
      NOW,
    );
    incrementLegacyFallbackAttribution(
    ...
    expect(fallbackAttributionRowCount(db)).toBe(1);
    expect(listLegacyFallbackAttributionSince(db, 'tenant-a', chatCoreV2HourBucket(NOW))).toEqual([
      {
        domain: 'tasks_comprar_suplementos',
        routeOwner: 'legacy_action_planner',
  ...
  it('tenant isolation: attributed rows never cross tenants', () => {
    incrementLegacyFallbackAttribution(db, 'tenant-a', { fellBack: true, domain: 'tasks' }, NOW);
    incrementLegacyFallbackAttribution(db, 'tenant-b', { fellBack: false, domain: 'tasks' }, NOW);
    ...
    expect(listLegacyFallbackAttributionSince(db, 'tenant-b', chatCoreV2HourBucket(NOW))).toEqual([
      {
        domain: 'tasks',
        routeOwner: 'unknown',
        routeMethod: 'unknown',
...
describe('autorevert-counters — fire-and-forget (never throws)', () => {
  ...
  it('incrementLegacyFallbackAttribution returns false (does NOT throw) on a closed db', () => {
    const db = new Database(':memory:');
    ensureChatCoreV2LegacyFallbackAttributionCounterTable(db);
    db.close();
    let result: boolean | undefined;
    expect(() => {
      result = incrementLegacyFallbackAttribution(db, 'tenant-a', { fellBack: true, domain: 'tasks' }, NOW);
    }).not.toThrow();
    expect(result).toBe(false);
  });
});
