...
describe('Chat Core v2 prepass recall@k eval', () => {
  ...
  it('reports per-language recall buckets so aggregate recall cannot hide a weak locale', () => {
    const result = evaluatePrepassRecallAtK(
      [
        { message: 'en-hit', language: 'en', expectedCapabilityIds: ['tasks.create'] },
        { message: 'pt-miss', language: 'pt-PT', expectedCapabilityIds: ['tasks.create'] },
        { message: 'mixed-hit', language: 'mixed', expectedCapabilityIds: ['tasks.create'] },
      ],
      8,
      (message) => (message === 'pt-miss' ? ['general.help'] : ['tasks.create']),
    );

    expect(result.recallAtK).toBeCloseTo(2 / 3, 10);
    expect(result.byLanguage.en).toEqual({ total: 1, scored: 1, hits: 1, recallAtK: 1 });
    expect(result.byLanguage['pt-PT']).toEqual({ total: 1, scored: 1, hits: 0, recallAtK: 0 });
    expect(result.byLanguage.mixed).toEqual({ total: 1, scored: 1, hits: 1, recallAtK: 1 });
    expect(result.misses[0].language).toBe('pt-PT');
  });
