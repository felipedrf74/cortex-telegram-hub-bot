...
const NON_SYNTHETIC_HASH = 'a'.repeat(64);
const PEER_REVIEW_SIGNOFF_HASH = 'b'.repeat(64);
const PEER_REVIEWED_NON_SYNTHETIC_HASH = buildPeerReviewedCorpusContentHash({
  corpusContentHash: NON_SYNTHETIC_HASH,
  peerReviewSignoffHash: PEER_REVIEW_SIGNOFF_HASH,
});
const SCHEMA_NOW = new Date('2026-05-30T12:00:00.000Z');

/** Seed N clean, safe-shaped shadow rows plus planner schema-compliance samples. */
...
describe('Chat Core v2 gate-metrics store (WP-13)', () => {
  ...
  describe('gateCanPromote truth table (HONEST)', () => {
    ...
    it('FALSE when shadow readiness is unmet, even with a peer-reviewed recall >= target', () => {
      // Only a few shadow rows (< 50) -> shadow readiness NOT met.
      seedSafeShadowTurns(3);
      upsertRecallAt8(0.95, { corpusContentHash: PEER_REVIEWED_NON_SYNTHETIC_HASH }, db);

      const report = measureChatCoreV2ShadowGateReadiness(db);
      expect(report.shadow.meetsMinRows).toBe(false);
      expect(report.recallMeetsTarget).toBe(true); // recall itself is fine
      expect(report.recallPeerReviewedCorpus).toBe(true);
    ...
    it('FALSE when the persisted recall is bound to the synthetic-seed hash (synthetic can never promote)', () => {
      ...
      expect(report.recallPeerReviewedCorpus).toBe(false);
    ...
    it('FALSE when a non-synthetic recall is not bound to peer-review signoff', () => {
      seedSafeShadowTurns(50);
      upsertRecallAt8(0.95, { corpusContentHash: NON_SYNTHETIC_HASH }, db);

      const report = measureChatCoreV2ShadowGateReadiness(db);
      expect(report.shadow.meetsMinRows).toBe(true);
      expect(report.persistedRecallAt8).toBe(0.95);
      expect(report.recallBoundToSyntheticSeed).toBe(false);
      expect(report.recallPeerReviewedCorpus).toBe(false);
      expect(report.recallMeetsTarget).toBe(false);
      expect(report.gateCanPromote).toBe(false);
      expect(report.notes).toContain('peer-reviewed corpus signoff');
    });

    it('FALSE when a peer-reviewed recall is below the 0.90 target', () => {
      seedSafeShadowTurns(50);
      upsertRecallAt8(0.85, { corpusContentHash: PEER_REVIEWED_NON_SYNTHETIC_HASH }, db);
    ...
    it('TRUE only when shadow readiness met AND a peer-reviewed non-synthetic recall >= target', () => {
      seedSafeShadowTurns(50);
      upsertRecallAt8(0.91, {
        corpusContentHash: PEER_REVIEWED_NON_SYNTHETIC_HASH,
        byLanguage: {
          en: { recallAtK: 0.99 },
          'pt-BR': { recallAtK: 0.98 },
          'pt-PT': { recallAtK: 0.93 },
          mixed: { recallAtK: 0.91 },
        },
      }, db);
      ...
      expect(report.recallMeetsLanguageTargets).toBe(true);
  ...
  describe('synthetic-seed hash binding', () => {
    ...
    it('binds a raw corpus hash to a peer-review signoff hash as a safe scalar', () => {
      expect(isPeerReviewedCorpusContentHash(NON_SYNTHETIC_HASH)).toBe(false);
      expect(isPeerReviewedCorpusContentHash(PEER_REVIEWED_NON_SYNTHETIC_HASH)).toBe(true);
      expect(rawCorpusContentHashFromGateBinding(PEER_REVIEWED_NON_SYNTHETIC_HASH)).toBe(NON_SYNTHETIC_HASH);
      expect(rawCorpusContentHashFromGateBinding(NON_SYNTHETIC_HASH)).toBe(NON_SYNTHETIC_HASH);
    });
  });
