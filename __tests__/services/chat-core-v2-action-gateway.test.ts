...
describe('ChatCoreV2 action gateway', () => {
  ...
  it('does not treat task-status read questions as completion writes', () => {
    for (const text of [
      ...
      'What tasks are complete today?',
    ]) {
      expect(detectChatCoreV2WriteIntent(text)).toEqual(expect.objectContaining({
        mayMutate: false,
        detectedIntent: 'none',
        reasonCodes: ['read_question_task_status'],
      }));
    }

    expect(detectChatCoreV2WriteIntent('Mark my task complete')).toEqual(expect.objectContaining({
      mayMutate: true,
      detectedIntent: 'task_complete',
      actionType: 'tasks.complete',
    }));
  ...
  it('flags bare ambiguous cancel/move/postpone as a write intent (not a free-text answer)', () => {
    for (const text of [
      ...
      'cancela isso',
      'move it pra sexta',
      'Adia o meu treino de hoje para amanhã',
      'muda essa para amanhã',
  ...
  it('blocks localized finance payment/tax intents before deterministic-read fallback', () => {
    for (const text of [
