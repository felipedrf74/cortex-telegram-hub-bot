...
describe('ChatCoreV2 activation contracts', () => {
  ...
  it('keeps the master kill switch authoritative over every other flag', () => {
    ...
    expect(config.legacyFallbackDisabled).toBe(false);
  ...
  it('keeps absent mode off but lets explicit full mode default to write execution and all domains', () => {
    const absent = resolveChatCoreV2ActivationConfig({});
    expect(absent.mode).toBe('off');
    expect(absent.allowedDomains).toEqual([]);
    expect(absent.allowWriteExecution).toBe(false);

    const canary = resolveChatCoreV2ActivationConfig({
      CHAT_CORE_V2_ORCHESTRATOR_MODE: 'canary',
    });
    expect(canary.allowWriteExecution).toBe(false);
    expect(canary.allowedDomains).toEqual(['training', 'cooking', 'content', 'finance']);

    const full = resolveChatCoreV2ActivationConfig({
      CHAT_CORE_V2_ORCHESTRATOR_MODE: 'on',
    });
    expect(full.allowWriteExecution).toBe(true);
    expect(full.allowedDomains).toEqual([
      'secretary',
      'tasks',
      'training',
      'content',
    ...
    expect(resolveChatCoreV2ActivationConfig({
      CHAT_CORE_V2_ORCHESTRATOR_MODE: 'on',
      CHAT_CORE_V2_ALLOW_WRITE_EXECUTION: 'false',
    }).allowWriteExecution).toBe(false);
  });

  it('resolves legacy fallback retirement per tenant and keeps the kill switch authoritative', () => {
    const env = {
      CHAT_CORE_V2_ORCHESTRATOR_MODE: 'canary',
    ...
    expect(resolveChatCoreV2ActivationConfig(env).legacyFallbackDisabled).toBe(true);
    expect(isChatCoreV2LegacyFallbackDisabledForTenant(env, 'tenant-a')).toBe(true);
    expect(isChatCoreV2LegacyFallbackDisabledForTenant(env, 'tenant-b')).toBe(false);

    setChatCoreV2RuntimeOverride('tenant-b', { legacyFallbackDisabled: true });
    expect(isChatCoreV2LegacyFallbackDisabledForTenant(env, 'tenant-b')).toBe(true);

    setChatCoreV2RuntimeOverride('tenant-a', { legacyFallbackDisabled: false });
    expect(isChatCoreV2LegacyFallbackDisabledForTenant(env, 'tenant-a')).toBe(false);

    expect(isChatCoreV2LegacyFallbackDisabledForTenant({
      ...env,
      CHAT_CORE_V2_ORCHESTRATOR_MODE: 'off',
    }, 'tenant-b')).toBe(false);
  });
  ...
  it('rejects cloud allowlist packets with unknown enum values even if the shape looks safe', () => {
    const result = buildCloudAllowlistPacket({
      enabled: true,
      budgetAvailable: true,
      tenantId: 'tenant-a',
      ...
      domain: 'content',
      entityRefs: [{ entityType: 'task', entityId: 'private-task-1' }],
      locale: 'en',
      complexityScore: 0.1,
    ...
    expect(result.ok).toBe(true);
    if (!result.ok) throw new Error('expected safe packet');
    expect(validateCloudAllowlistPacket({
      ...result.packet,
      intent: 'freeform_raw_chat' as any,
    })).toBe(false);
    expect(validateCloudAllowlistPacket({
      ...result.packet,
      domain: 'calendar_private_raw' as any,
    })).toBe(false);
    expect(validateCloudAllowlistPacket({
      ...result.packet,
    ...
    expect(validateCloudAllowlistPacket({
      ...result.packet,
      rawMessage: 'crie uma tarefa privada',
    } as any)).toBe(false);
    expect(validateCloudAllowlistPacket({
      ...result.packet,
      hmacEntityIds: [{
        ...result.packet.hmacEntityIds[0],
        rawId: 'task-42',
      }],
    } as any)).toBe(false);
  });
  ...
  it('drops raw-looking cloud evidence fingerprints before packet construction', () => {
    const result = buildCloudAllowlistPacket({
      enabled: true,
      budgetAvailable: true,
      tenantId: 'tenant-a',
      ...
      capabilityId: 'general.help',
      domain: 'content',
      evidenceFingerprints: [' raw customer calendar text ', 'evidence:general-focus'],
      locale: 'en',
      complexityScore: 0.1,
      escalationReason: 'cloud_allowlist_candidate',
    });
    ...
    expect(result.ok).toBe(true);
    if (!result.ok) throw new Error('expected safe packet');
    expect(result.packet.evidenceFingerprints).toEqual(['evidence:general-focus']);
    expect(JSON.stringify(result.packet)).not.toContain('customer calendar');
  });
  ...
  it('pairs repair-only pin with tenant shadow demotion on a schema-compliance breach', () => {
    ...
    // The pin is consumed by the shadow planner path, where it skips fresh
    // planner generation for that tenant after demotion.
