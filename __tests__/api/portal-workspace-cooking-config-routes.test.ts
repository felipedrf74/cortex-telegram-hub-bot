...
/**
 * Route tests for Cooking skill config (OI-DATA-003d, 2026-04-23).
...
describe('Cooking config routes (OI-DATA-003d)', () => {
  ...
  it('GET /workspace/skills/cooking/config returns 6 default fields + schemaKeys', async () => {
    const r = await req(app, 'GET', '/workspace/skills/cooking/config', { userId: alice });
    expect(r.status).toBe(200);
    expect(r.body.data.skillId).toBe('cooking');
    expect(r.body.data.config.serving_size).toBe('2');
    expect(r.body.data.config.meal_cost_ceiling).toBe('moderate');
  ...
  it('PUT valid Cooking body saves', async () => {
    const r = await req(app, 'PUT', '/workspace/skills/cooking/config', {
      userId: alice, tenantId: String(alice),
      body: { config: {
        dietary_restrictions: 'Tree nut allergy',
        serving_size: '4',
        meal_cost_ceiling: 'budget',
      } },
    });
    expect(r.status).toBe(200);
    expect(r.body.data.config.dietary_restrictions).toBe('Tree nut allergy');
    expect(r.body.data.config.serving_size).toBe('4');
    expect(r.body.data.config.meal_cost_ceiling).toBe('budget');
  });

  it('PUT with Content-only field (voice_guidelines) → 400 with Cooking allowed fields', async () => {
    const r = await req(app, 'PUT', '/workspace/skills/cooking/config', {
      userId: alice, tenantId: String(alice),
    ...
    expect(r.body.error.details?.allowed).toEqual(expect.arrayContaining([
      'dietary_restrictions', 'preferences', 'kitchen_inventory',
      'serving_size', 'meal_cost_ceiling', 'extra_notes',
    ]));
  });

  it('Home dep cooking.restrictions.set flips missing → ready → missing on set/clear', async () => {
    let h = await req(app, 'GET', '/workspace/console/home', { userId: alice });
    let dep = h.body.data.dependencies.items.find((d: any) => d.id === 'cooking.restrictions.set');
    expect(dep).toBeTruthy();
    expect(dep.status).toBe('missing');
    expect(dep.cta.href).toBe('#/skills/cooking/configuration');

    await req(app, 'PUT', '/workspace/skills/cooking/config', {
      userId: alice, tenantId: String(alice),
      body: { config: { dietary_restrictions: 'Tree nut allergy' } },
    });
    h = await req(app, 'GET', '/workspace/console/home', { userId: alice });
    dep = h.body.data.dependencies.items.find((d: any) => d.id === 'cooking.restrictions.set');
    expect(dep.status).toBe('ready');
    expect(dep.cta).toBeNull();

    await req(app, 'PUT', '/workspace/skills/cooking/config', {
      userId: alice, tenantId: String(alice),
      body: { config: { dietary_restrictions: '' } },
    });
    h = await req(app, 'GET', '/workspace/console/home', { userId: alice });
    dep = h.body.data.dependencies.items.find((d: any) => d.id === 'cooking.restrictions.set');
    expect(dep.status).toBe('missing');
  ...
  it('Home dep is gated on restrictions NOT preferences (safety check)', async () => {
    // Setting only preferences (soft) should leave the dep MISSING.
    // Restrictions are hard constraints — safety-relevant for allergies.
    await req(app, 'PUT', '/workspace/skills/cooking/config', {
      userId: alice, tenantId: String(alice),
      body: { config: { preferences: 'love Thai, medium spice' } },
    });
    const h = await req(app, 'GET', '/workspace/console/home', { userId: alice });
    const dep = h.body.data.dependencies.items.find((d: any) => d.id === 'cooking.restrictions.set');
    expect(dep.status).toBe('missing');
  });
});
