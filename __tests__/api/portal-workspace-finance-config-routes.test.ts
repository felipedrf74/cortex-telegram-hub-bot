...
/**
 * Route tests for Finance skill config (OI-DATA-003c, 2026-04-23).
 * Mirrors the Secretary/Training route test shape.
 */
...
describe('Finance config routes (OI-DATA-003c)', () => {
  ...
  it('GET /workspace/skills/finance/config returns 6 default fields + schemaKeys', async () => {
    const r = await req(app, 'GET', '/workspace/skills/finance/config', { userId: alice });
    expect(r.status).toBe(200);
    expect(r.body.data.skillId).toBe('finance');
    expect(r.body.data.config.primary_currency).toBe('USD');
    expect(r.body.data.config.decision_style).toBe('balanced');
  ...
  it('PUT valid Finance body saves', async () => {
    const r = await req(app, 'PUT', '/workspace/skills/finance/config', {
      userId: alice, tenantId: String(alice),
      body: { config: {
        budget_monthly: '$4,500 monthly.',
        primary_currency: 'EUR',
        decision_style: 'conservative',
      } },
    });
    expect(r.status).toBe(200);
    expect(r.body.data.config.budget_monthly).toBe('$4,500 monthly.');
    expect(r.body.data.config.primary_currency).toBe('EUR');
    expect(r.body.data.config.decision_style).toBe('conservative');
  });

  it('PUT with Content-only field (voice_guidelines) → 400 with Finance allowed fields', async () => {
    const r = await req(app, 'PUT', '/workspace/skills/finance/config', {
      userId: alice, tenantId: String(alice),
    ...
    expect(r.body.error.details?.allowed).toEqual(expect.arrayContaining([
      'budget_monthly', 'saving_goals', 'affordability_rules',
      'primary_currency', 'decision_style', 'extra_notes',
    ]));
  });

  it('Home dep finance.budget.set flips missing → ready → missing on set/clear', async () => {
    let h = await req(app, 'GET', '/workspace/console/home', { userId: alice });
    let dep = h.body.data.dependencies.items.find((d: any) => d.id === 'finance.budget.set');
    expect(dep).toBeTruthy();
    expect(dep.status).toBe('missing');
    expect(dep.cta.href).toBe('#/skills/finance/configuration');

    await req(app, 'PUT', '/workspace/skills/finance/config', {
      userId: alice, tenantId: String(alice),
      body: { config: { budget_monthly: '$5k' } },
    });
    h = await req(app, 'GET', '/workspace/console/home', { userId: alice });
    dep = h.body.data.dependencies.items.find((d: any) => d.id === 'finance.budget.set');
    expect(dep.status).toBe('ready');
    expect(dep.cta).toBeNull();

    await req(app, 'PUT', '/workspace/skills/finance/config', {
      userId: alice, tenantId: String(alice),
      body: { config: { budget_monthly: '' } },
    });
    h = await req(app, 'GET', '/workspace/console/home', { userId: alice });
    dep = h.body.data.dependencies.items.find((d: any) => d.id === 'finance.budget.set');
    expect(dep.status).toBe('missing');
  ...
  it('cost-privacy invariant holds with Finance config in the mix', async () => {
    ...
    await req(app, 'PUT', '/workspace/skills/finance/config', {
      userId: alice, tenantId: String(alice),
      body: { config: { budget_monthly: 'private budget' } },
    });
