...
vi.mock('../../src/portal/admin-target-user', () => ({ requireOperatorTargetUser: hoisted.requireOperatorTargetUser,
  authorizePortalOperatorTargetUser: vi.fn(),
  getPortalAdminTargetUserId: vi.fn(),
  isOperatorScopedToUser: vi.fn(),
  portalOperatorUserScopesConfigured: vi.fn(),
}));
...
vi.mock('../../src/services/notification-orchestrator', () => ({ revokeNotificationDeviceToken: hoisted.revokeNotificationDeviceToken,
  DIGEST_MAX_SLOTS: vi.fn(),
  DIGEST_SURFACED_WINDOW_HOURS: vi.fn(),
  INTERRUPT_GLOBAL_DAILY_CAP: vi.fn(),
  INTERRUPT_SKILL_DAILY_CAP: vi.fn(),
  INTERRUPT_TIER_DAILY_CAPS: vi.fn(),
  NEW_USER_RAMP_DAILY_CAP: vi.fn(),
  NEW_USER_RAMP_DAYS: vi.fn(),
  NOTIFICATION_RETENTION_DAYS: vi.fn(),
  NOTIFICATION_TITLE_KEYS: vi.fn(),
  NotificationProposalCommitError: vi.fn(),
  PROMOTIONAL_MIN_DAYS_BETWEEN: vi.fn(),
  SNOOZE_DEFAULT_MINUTES: vi.fn(),
  SNOOZE_MAX_COUNT: vi.fn(),
  SNOOZE_MAX_DAYS: vi.fn(),
  SNOOZE_MIN_MINUTES: vi.fn(),
  applyNotificationProfilePatch: vi.fn(),
  assembleDailyDigest: vi.fn(),
  buildApnsCollapseId: vi.fn(),
  buildSkillNotificationFixtureIntent: vi.fn(),
  countUnreadNotificationCenterItems: vi.fn(),
  createNotificationIntent: vi.fn(),
  dismissNotificationCenterItem: vi.fn(),
  ensureNotificationTables: vi.fn(),
  evaluateInterruptBudget: vi.fn(),
  evaluateNotificationIntent: vi.fn(),
  expireStaleNotificationIntents: vi.fn(),
  getAllNotificationCenterItemsForPortal: vi.fn(),
  getNotificationCenterItem: vi.fn(),
  getNotificationDecisionLog: vi.fn(),
  getNotificationDeliveryObservabilityMetrics: vi.fn(),
  getNotificationProfileIfExists: vi.fn(),
  getNotificationProfileSummariesForPortal: vi.fn(),
  getNotificationReliabilityDashboard: vi.fn(),
  getOrCreateNotificationProfile: vi.fn(),
  interruptTierFor: vi.fn(),
  isNotificationDeliverableForProfile: vi.fn(),
  isNotificationPushableForProfile: vi.fn(),
  listNotificationBridgeEntityIds: vi.fn(),
  listNotificationCenterItems: vi.fn(),
  markDeviceTokensUnregistered: vi.fn(),
  markNotificationCenterItemRead: vi.fn(),
  notificationCopyLanguage: vi.fn(),
  notificationDecisionReachedUser: vi.fn(),
  notificationDeliveryFailed: vi.fn(),
  notificationIntentDeliveryScopeKey: vi.fn(),
  notificationPushBlockCause: vi.fn(),
  notificationReachability: vi.fn(),
  notificationTimezoneDrift: vi.fn(),
  notificationTitleKey: vi.fn(),
  notificationTitleOrFallback: vi.fn(),
  performNotificationAction: vi.fn(),
  processNotificationIntentDelivery: vi.fn(),
  pruneNotificationRetention: vi.fn(),
  pruneStaleDeviceTokens: vi.fn(),
  recordNotificationEngagementEvent: vi.fn(),
  recordNotificationReliabilityEvent: vi.fn(),
  registerNotificationDeviceToken: vi.fn(),
  releaseDueNotificationDeliveries: vi.fn(),
  releaseDueSnoozedNotifications: vi.fn(),
  resolveSnoozeUntil: vi.fn(),
  resumeNotificationIntentDelivery: vi.fn(),
  sanitizeNotificationDeliveryErrorCode: vi.fn(),
  snoozeNotificationCenterItem: vi.fn(),
  stripTitleEmoji: vi.fn(),
  touchDeviceTokenActivity: vi.fn(),
  updateNotificationProfile: vi.fn(),
  userHasActivePushDeviceToken: vi.fn(),
}));
vi.mock('../../src/services/integration-status', () => ({ getIntegrationSummary: hoisted.getIntegrationSummary,
  ALL_INTEGRATION_PROVIDERS: vi.fn(),
  capabilitiesForProvider: vi.fn(),
  getProviderStatus: vi.fn(),
  hasUsableCalendarProvider: vi.fn(),
  hasUsableHealthProvider: vi.fn(),
  hasUsableMailProvider: vi.fn(),
  isGarminActivelyIntegrated: vi.fn(),
}));
