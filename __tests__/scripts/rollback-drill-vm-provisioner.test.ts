...
describe('rollback-drill KVM provisioner', () => {
  it('selects exactly one owner-reviewed image from a strict signed-manifest payload', () => {
    ...
    const selected = spawnSync('python3', [
      manifestHelper,
      '--checksums',
      sums,
      '--filename',
  ...
  ])('rejects $name after signature verification', ({ lines, expected, error }) => {
    const root = temporaryRoot();
    const sums = join(root, 'SHA256SUMS');
  ...
  it('installs only an exact root-owned compatibility set and leaves every guest static and inactive', () => {
    ...
    expect(installer).toContain('EXPECTED_BOOTSTRAP_ROOT="$BOOTSTRAP_BASE/$SOURCE_SHA"');
    ...
    expect(installer).toContain('install layout differs from the exact allowlist');
    ...
    expect(installer).toContain('must belong only to its private group and kvm');
    expect(installer).toContain('assert_template_static false');
    expect(installer).toContain('systemd-analyze verify "$unit_verify_path"');
    expect(installer).toContain('compile(path.read_bytes(), str(path), "exec")');
    expect(installer).toContain(
    ...
    expect(unit).toContain('ConditionPathExists=!/var/lib/nexus-rollback-drill-vm/install-in-progress.v1');
    expect(unit).toContain('ConditionPathExists=!/var/lib/nexus-rollback-drill-vm/provision-in-progress.v1');
  ...
  it('enforces the bounded host admission floor before a VM can start', () => {
    ...
    const verify = (availableKiB: number, load15: string) => {
      writeFileSync(meminfoPath, `MemTotal: 32500000 kB\nMemAvailable: ${availableKiB} kB\n`);
  ...
  it('rejects replaced, spoofed, and contended inherited single-guest locks', () => {
    ...
    expect(exercise('10:20', '10:20', 0, 'root:nexus-drill-vm:660:2').status)
      .not.toBe(0);
  });
  ...
  it('root-copies an optional unprivileged cache before independently verifying Canonical identity', () => {
    ...
    expect(provisioner).toContain(
      '--max-filesize "$expected_image_size"',
    );
    ...
    expect(provisioner).toContain('downloaded image is not the expected standalone qcow2 format');
    expect(provisioner).toContain(
      'base_target="$BASE_DIR/$expected_image_sha256.qcow2"',
    );
    expect(provisioner).not.toMatch(/\b(?:bash|sh|source|\.)\s+["']?\$staged_source_directory/);
  });
  ...
  it('publishes exactly three independent key-only guests in one atomic set', () => {
    ...
    expect(provisioner).toContain('"overlayInitialSha256": overlay_initial_sha');
    ...
    expect(provisioner).toContain('mv -fT -- "$active_stage" "$ACTIVE_RECEIPT"');
  ...
  it('runs one guest at a time with loopback-only user networking and regular-file drives', () => {
    ...
    expect(runner).toContain('guest host-key fingerprint does not match its public key');
    ...
    expect(runner).toContain('guest overlay is not pristine; provision a fresh set');
  ...
  it('behaviorally refuses a selected overlay after any byte-level mutation', () => {
    ...
    const exercise = () => spawnSync(
      ...
      ],
      {
        ...
        env: {
          ...process.env,
          overlay_initial_sha256: expected,
    ...
    expect(rejected.stderr).toContain(
      'guest overlay is not pristine; provision a fresh set',
    );
  });
  ...
  it('documents the fail-closed owner boundary without claiming drill completion', () => {
    ...
    expect(operations).toContain('/home/dominguez/.npm-global/bin/pm2');
    expect(operations).toContain('This first version deliberately has no automatic guest deletion');
  ...
  it('keeps privileged assets at the executable modes declared by the install layout', () => {
    for (const [path, expectedMode] of [
      ...
      expect(mode, `${path} mode differs from its install contract`).toBe(expectedMode);
    }
  });
});
