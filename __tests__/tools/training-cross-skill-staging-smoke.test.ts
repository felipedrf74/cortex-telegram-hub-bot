...
describe('training cross-skill staging smoke harness', () => {
  ...
  it('passes runtime flows only when scoped staging fixture signals are present and deduped', async () => {
    const report = await runTrainingCrossSkillStagingSmoke({
      ...
      contentSignals: [signal('content_capture_opportunity', {
        date: '2026-05-07',
        status: 'proposed',
      ...
      sharedDecisionContext: [
        ...
        'Content: a Thursday filming window is proposed and remains unreserved.',
      ].join('\n'),
    }));
    ...
    async readContentMeshContext() {
      return {
        ...
        workSchedule: {
          ...
          planStatus: opts.contentSignals.length > 0 ? 'proposed' : 'unplanned',
        ...
        monitoredPillars: [],
        recentSignals: [],
        nextExecution: opts.contentSignals.length > 0 ? {
          mode: 'film_window',
          title: 'Training block update',
          summary: 'Recommended Content work only.',
...
async function buildPassingSmokeReport() {
  return runTrainingCrossSkillStagingSmoke({
    ...
    sharedDecisionContext: [
      ...
      'Content: a filming proposal is active; Secretary has not reserved it.',
    ].join('\n'),
  }));
}
